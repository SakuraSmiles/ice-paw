...
| 7+ | S3 格式与编号循环（使用驱动，每件一 commit） | **首波四件（D8 拍板 2026-08-24）**：① numbering.xml 接入——解析编号定义 + 计数模拟，outline/format 显示自动编号实际值（治「agent 看不见 3.2.1 是几」盲区，读侧地基）② set_format 操作（字符/段落格式：字号/加粗/颜色/对齐/行距/缩进）③ set_style 操作（已有段换样式，标题升降级）④ 页眉页脚读（header/footer 部件进 inspect）。后续波：TOC（域+诚实提示刷新）/图片插入/文档级条件批量替换/表格高级 | 渐进 |
