...
| 3 | **zip 手术引擎 + edit_docx MVP**（改优先核心，提前于此） | 手术引擎（只替换目标部件、其余 entry 字节原样重打包 + 字节相等断言）；edit_docx 批量事务 + 地址指纹核对 + 模型级 diff 读回；操作词表 v1（改文本/插删段/字符格式/段落格式/表格行列）。真机验收：「标题改蓝、第 N 段后插一段、表格加一行」全对、diff 准确、untouched 部件字节相等 | 大 |
...
| D3 | 工具粒度 | 批量事务（倾向，步骤 3 开工前终拍） | 粗粒度：token 省、轮数少、天然原子 |
