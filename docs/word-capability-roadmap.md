...
| 5 | **S1a write_docx 模板优先生成** ✅ | 内容块模型（heading/paragraph/table，rows_text 值优先）+ 模板三层（内置档位 / workspace templates/ 相对路径 / 绝对路径）+ 生成自检（不过不落盘）+ 备份/原子写；真机验收：agent 生成报告，Word/WPS 打开排版合格 | 中 ✅（2026-08-31 九波 D16 改道模板优先，纯创建降级延后：内置占位模板代码内建（`report`，用户 Word 造正式模板后替换）；编排 = clear_body→注入裸锚→正序批次（连续段聚批、表独批）→删锚，全内存链复用已验证手术引擎；自检 = validate 引擎逐块断言（段=块文本全等、表=行列数+逐格），chunk 50；指路文案升级（edit_docx description 模板行 + help/agent-yaml.md）；详见 D16） |
| 6 | S1b 内置模板资产扩充 + 模板捐赠验收 | 用户用 Word 造 2-3 个正式模板替换占位（演进为 include_bytes! 文件表，一行一事）；拿用户真实模板（workspace templates/ 与绝对路径两层已随 D16 打通）生成，样式一致 = 真机验收 | 小 |
...
| D4 | 模板传递形态 | 对话给路径起步（步骤 6 前终拍） | **已定三层阶梯（D16 落地）**：内置档位名 / workspace templates/ 相对路径 / 绝对路径——`template` 单参数统一承载，项目级约定与任意路径捐赠都覆盖 |
