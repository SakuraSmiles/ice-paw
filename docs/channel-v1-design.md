...
- **ChatMessages**：`MessageGroup` merge 条件（`ChatMessages.vue:697-701` 现为 `assistant && 同 model`）
  **加 sender 维度**——不同成员的回复不互相吞并；每消息组**群聊气泡布局**（2026-09-11 拍板改版：头像
  `EntityAvatar`（lg 36px）独立于气泡外左列 + 成员名字（body-sm）在气泡上方 + 气泡体承接组容器背景——
  内容与 1v1 一致；最初版「sm 头像 + caption 名字嵌气泡内」已被替换）+ 统筹者 shield 微标 + **组级时间
  「开始 → 完成」**（相对时 + hover 绝对时，C8b derive 透传的时长字段）；**用户消息落在前一回合区间内 →
  气泡标注「生成中发出」**（C8b）。系统事实提示条
