...
| 委派（成员回合内调 delegate） | 照常，深度护栏不变（见 C7） |
...
### C7 频道回合允许委派（深度 1 与 1v1 一致）

- 委派深度护栏是结构性的：`delegate` 工具按会话 kind 注册（`session_runner.rs:553-560` 现判定 `conv.kind == "chat"`，
  委派子会话 `kind='delegation'` 拿不到工具 → 深度恒 1）。**频道接入 = 判定放开为
  `conv.kind == "chat" || conv.kind == "channel"`**，护栏天然保持，零新机制。
- 委派是**同步工具**（结果经 tool_result 回流，回合内完成）——「委派完成回流」无需系统事实消息唤醒，
  成员 X 委派 Y 的子会话完成后 X 在同一回合内拿到结果继续。**之前讨论过的「异步回流唤醒」不成立，勿实现。**
- 频道回合预算归**执行成员**：`LoopBudget` 是每回合局部变量、数值全部取自 `AgentTurnInput.agent` 行配置
  （`session_runner.rs:486-500`，无 per-conversation 累计）——天然正确，零改动。
- 降级链继承（0.7 批 A `delegation_fallback_plan`）与 L2 委派预授权对频道回合自动生效（同一 `run_agent_turn` 链路）。
...
- **工具面**：`delegate` 注册放开（C7）；`send_message_to_session` / `list_conversations` **不注册给频道回合**
  （v1 砍法——频道内 @ 是原生通讯方式，跨会话投递从频道发起留 v2；`inbox.rs:314` deliver 拦 `kind != "chat"`
  与 drain watcher `kind != "chat"` 判定**维持现状**——频道不收投递，防侧信道绕过频道护栏，delegation 同款逻辑）。
