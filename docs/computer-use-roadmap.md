...
## 现状：批次③ 看屏+操作引擎（✅ 2026-08-28 落地；已随 0.6.0 发版 push 2026-08-31，未手测）
...
### act-and-look 操作后自动附图（✅ 2026-08-29 落地；已随 0.6.0 发版 push 2026-08-31，未手测）

生产反馈：agent 操作不连贯——截图→思考→移动→思考→点击→再截图确认，每步一整轮 LLM 往返，且坐标在 t0 计算到 t5 执行之间画面可能已变。治法 = 把「动作 → 视觉反馈」合一（Anthropic computer-use 官方模式）：

- **六件操作工具（move/click/drag/scroll/type/press）成功后统一走 `action_shot`**：初始 300ms 稳定等待后**等画面收敛**（隔 250ms 抓两帧逐字节比较，相同才稳定；总上限 2s）——办公场景「操作后被动变化」（开应用/切窗口/保存对话框 1-2s 收敛）附到最终态而非半开中间态；到上限仍不稳（持续动画）如实附当前帧并在 note 标注「仍在变化」指路 wait。稳定帧附进同一 tool_result → **即刻写为新坐标基准**（下一坐标天然基于最新画面，结构性消除过期坐标）。捕获同源：与上一张基准图同一物理区域取交集（窗口基准→同矩形；无基准→整虚拟桌面），像素空间连续；等稳定观察帧也抓同一矩形（region 在循环外定死）。静态桌面固定成本 ~550ms（初始等待+一轮比较）。
- **描述行为引导**（刀2 随手落）：click 声明「move+click 一步完成，勿先 mouse_move」；六件声明「结果自带操作后截图，勿再 capture_screen 只为确认」；摘要 note 声明「本图即最新基准，下一步坐标用它的像素空间」。
- **wait 刻意保持纯文本**：它是 Always 级（无外部作用才免审批），静默附图 = 画面在用户不知情下离开本机——授权治理优先于节奏便利。六件 auth_reason 已同步披露「并回传操作后的屏幕截图给当前模型服务商」。
- **失败降级不污染家族前缀**：附图失败（捕获/编码错）只 warn tracing，工具结果仍成功 + note 指路 capture_screen——doom_detect 依赖错误首行稳定。
- **token 增长由既有双钩兜住**：compact_screenshot_history keep_last_k=3 只保最近 3 张在 LLM 视图，连续操作的附图洪峰不会爆上下文（DB/事件日志仍保完整图）。
...
- [ ] act-and-look 真机手测——**常规桌面场景**（游戏场景用户 2026-08-29 拍板整体搁置：act-and-look 落地后真机测游戏仍不满足，需求不迫切）：操作连贯性提升验证（多步任务中 agent 是否不再逐步 capture 确认）+ 附图延迟感受（每操作 +300ms）+ token 消耗对比
- [ ] **光标三件套（游戏场景搁置范围内，勿主动拾起；游戏内皮肤光标与系统光标错位，agent 看不到自己的光标）**：先做 1 分钟实验定分叉（GDI BitBlt 从不捕获硬件光标层——是硬件黑障还是识别问题）；A 捕获合成系统光标（CURSORINFO 叠画进帧）/ B 视觉伺服 mouse_servo（frame-diff：小步移动→帧间唯一动体即光标，闭环到目标）/ C GetCursorPos 落位真值（echo 光标位置供模型校验）。raw-input 游戏需相对位移步进而非绝对坐标
- [ ] 批次③ 真机手测——自动化冒烟已绿（坐标链偏差 0/三路捕获/GDI 字节序契约），剩余人工面：中文输入手感/拖拽/点击滚轮/组合键/wait 中断 + 多显示器与 DPI 缩放（本机单屏 96 DPI，需换硬件）——不阻塞批次④ 开发
- [x] 批次④ 按提交切分 1-5 推进（步骤 1-5 已全落地，各步落地形态见 §4.12；真机走查见下条）
