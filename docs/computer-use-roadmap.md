...
- [ ] act-and-look 真机手测——操作连贯性提升验证（多步任务中 agent 是否不再逐步 capture 确认）+ 附图延迟感受（每操作 +300ms）+ token 消耗对比
- [ ] **光标三件套（用户挂起 2026-08-29：游戏内皮肤光标与系统光标错位，agent 看不到自己的光标）**：先做 1 分钟实验定分叉（GDI BitBlt 从不捕获硬件光标层——是硬件黑障还是识别问题）；A 捕获合成系统光标（CURSORINFO 叠画进帧）/ B 视觉伺服 mouse_servo（frame-diff：小步移动→帧间唯一动体即光标，闭环到目标）/ C GetCursorPos 落位真值（echo 光标位置供模型校验）。raw-input 游戏需相对位移步进而非绝对坐标
