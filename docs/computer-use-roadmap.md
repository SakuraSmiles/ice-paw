...
- **六件操作工具（move/click/drag/scroll/type/press）成功后统一走 `action_shot`**：~300ms 稳定等待（重绘/菜单展开/跳转动画收敛）→ 抓「操作效果」图附进同一 tool_result → **即刻写为新坐标基准**（下一坐标天然基于最新画面，结构性消除过期坐标）。捕获同源：与上一张基准图同一物理区域取交集（窗口基准→同矩形；无基准→整虚拟桌面），像素空间连续。
