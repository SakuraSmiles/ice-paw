...
| P11 | **轮次导航条不稳**（用户 2026-08-17 真机报）✅ C1/C2 结构修复已执行 2026-08-17（e345879），③待真机观察 | 症状三连：①数字不准 ②定位失误 ③不更新。**根因锁定两个高置信**：①跳转落点 12px 与视位判定线 80px 错位——估高漂移时锚点落在线下 → activeTurn 读 N-1（②及①的跳后表现）；②锚点 SQL 单看 blocks 含 tool_result 子串即排除——用户正文粘贴含该字面量的 JSON 被误排除而前端照样渲染 → 轮号整体偏移（①的另一来源）。修复：落点对齐判定线上方 24px（THRESHOLD_PX 导出单一真相源）+ scrollend 漂移纠正（估高→真实高度换算 >32px 瞬时补滚）+ 删 guard 60 页上限（3000 条封顶静默失败）+ SQL 改「content 空 ∧ blocks 含 tool_result」合取对齐前端 isToolResultOnlyUser（Rust 回归用例钉死三形态）。③「不更新」最可能可见面孔已被①覆盖，未盲改——复现时用 tick tooltip 锚点 id（新增）采集定位。**手测通过才终勾** |
| P12 | **任务胶囊 + 计划面板重设计**（用户 2026-08-17 提出）✅ 已执行 2026-08-17（437a09b）待真机手测 | 落地：popover 双栏（左任务/右计划）各列 `min(320px,48vh)` 独立内滚 + `max-width: min(560px, calc(100vw-48px))` 防窄窗左溢 + 窄窗 flex-wrap 堆叠回纵向；删 MAX_ROWS=8 截断（列内滚动后全量可见）；胶囊文案组合化「任务 N · 计划 D/M」+ dot 脉冲扩为任务在跑或计划推进中；状态翻转行背景轻闪 ~1.1s（任务 running / 计划条目 status，flashKeys 整替保响应式）+ plan-mark 平滑过渡 + 任务行淡入（subtle 不弹跳）。MA-2 台账仍独立页面，胶囊保持轻量索引定位 |
