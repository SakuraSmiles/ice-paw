...
| W5 | **失败静默族残余**：batch_writer.rs:313 sender-drop 退出路径 flush 失败无日志（对照显式关闭路径有 warn——流式正文尾部无痕丢失面）/ channel.rs:932 选举换选时旧统筹降级 set_member_role 失败被 let _ 吞（可短暂双 coordinator，同函数升级路径有 warn）/ kb/ensure.rs:103 种子写失败静默 + 无条件「已创建」/ chat.ts:872 延迟删除失败静默（steer.rs:189 一项随 W1 批 marker 记账重写一并消除） | 中低 | 📋 |
