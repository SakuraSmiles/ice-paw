...
| V13 | **质量拍⑤ prompt 两层 + 风格预设**（2026-08-23，随下版发布） | （本次 commit） | ① 平台 base prompt 新稿（`system_prompt.rs`）：风格中立三条纪律——错误纪律（读恢复指引/勿原样重试，与①②④咬合）/诚实边界/语言跟随；「与你的人设叠加生效」锚两层关系；意图确认按用户圈改下沉工程档 ② 风格预设三档（工程/创作/陪伴，`data/stylePresets.ts` 前端常量）：素材不是档位——插入 agent.yaml 即用户文本 ③ 新命令 `set_agent_system_prompt`（`patch_agent_yaml_block` 块级补丁+回读闸+原子写；`set_agent_yaml_field` 仍 u64 标量专用）④ AgentForm 编辑态入口：徽章旁按钮+就地面板+覆盖确认（出生默认句豁免）。手测点：编辑 agent → 风格预设 → 插入 → 打开 agent.yaml 见多行块 + 现有注释/预算字段不动；新会话行为贴档（工程档先结论/陪伴档不催建议） |
