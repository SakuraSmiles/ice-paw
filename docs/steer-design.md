...
3. ~~过渡提示~~ **轻量版已落地（2026-09-19，用户真机反馈「插话后输入区瞬间回空闲态」驱动）**：静默窗（3s `CHAIN_HEAD_QUIET`）内输入区显示衔接态——ChatInput hint 行 mini spinner + 「插话排队中，即将继续…」+ wrapper 保留 `is-sending` 边框；判据 `!sending ∧ queuedSteerIds 命中当前会话消息`（messages 即当前会话列表 = 天然归属守卫，切走不显；回合 B 起跑 sending=true 即让位生成中形态）。不做假停止按钮（窗内确实无在途回合可停）；期间打字/连发照常（连发即再排队一条）。更精致的专门形态仍可后续设计，轻量版已是诚实信号非纯文字。回归锁 ChatInput.steer.test.ts ×4。
