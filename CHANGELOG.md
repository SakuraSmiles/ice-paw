...
- **MA-3 跨会话通讯（0.7 批 B）**：与委派互补的异步对等通道——agent 调 `send_message_to_session` 向任意会话投递消息，对方排队、空闲（或经批准）后消费一整回合；收件三态（accept 自动 / hold 扣住待批准（默认）/ refuse 拒收）；hold 来件走会话头收件箱 popover（批准/拒绝两步确认 + 政策切换），侧栏 badge 计数；`expect_reply` 回投链一次止；护栏：pending 上限 10、同会话 10 分钟窗口自动消费 ≤ 6 次。pending 只入事件队列不写 messages——与目标在途回合零冲突。
