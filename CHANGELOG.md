...
- **频道 v1（0.7 批 C，四批 commit）**：项目 1:1 的串行共享流会话——同一项目全体成员（project_agents）共用一条消息流，一时刻仅一成员发言，@点名必应答（支持多 @）、广播由统筹者编排、统筹者由系统自选举或用户指定、护栏截断接力风暴、人始终在场且治理权最大。
  - **零新表**：频道 = `conversations` 表 `kind='channel'` + `project_id` 必填的一行（migration 54 部分唯一索引保证一项目一活频道）；成员现读 project_agents，变更即时生效；侧栏「开启频道」入口懒建（项目创建不预建）。
  - **@ 点名双通路**：用户侧结构化 `mentions`（弹层选成员、文本保留 `@名字 ` 可见可删、发送时解析，与后端 `parse_agent_mentions` 同构——最长前缀精确匹配 / 重名歧义不猜 / email 防御 / 去重保序，上限 5）；成员侧由后端从回复文本解析。被 @ 成员跑一回合复用 run_agent_turn 全链路（预算/工具/hooks/事件照常）。
  - **广播与统筹者**：不 @ 的消息由统筹者接令——应答或 @成员 分派；统筹空缺时首次广播触发**一次性自选举**（成员互投小额度请求兼任健康检查，投票本身就是真实调用——连不上的成员天然弃权；全员弃权则空缺诚实披露）；两档治理——系统档故障保守换帅（接令回合连续失败 2 次罢免，投影回落 joined_at 最早成员），指定档故障不自动换帅（用户钦点不擅动，频道 popover 给「让系统补选」出口，可绕过弃权熔断）。
  - **护栏四件**：乒乓互 @（同一有序对发起者→被@者在一条链内至多 2 次）、接力链上限（单条链连续成员回合 ≤8）、成员唤醒频率闸（每成员 10 分钟窗 ≤6 次，用户点名不占）、用户插话优先（新链头打断在途接力链——在途发送**不拦截**，物化无条件成功，3s 静默窗口聚合连发「说完一起听」，真实还原群聊体验）；拦截事实落 `channel_mention` 事件（六 blocked_reason），不物化消息行（行为性事实只进事件日志，对 agent 告知走派生简报）。
  - **成员身份可视化**：消息按 sender 维度分组（不同成员绝不合并），身份头 = 发言时名快照（改名不追溯）+ 当前统筹者盾标；「生成中发出」标注（用户消息落在上一回合生成窗口内时标记——还原插话时序原貌）；频道行为事件（点名/选举/统筹位）渲染为时间线通知条，与消息按时间交错。
  - **频道归档（软删除）**：项目永久删除时频道归档保留（`archived_at`，记录只读：输入禁用 + 发送拒绝三段式），散落 scope 侧栏可见「已归档」行；删除成员的 agent 时其频道发言保留（投影迁移，历史不丢）。
  - **可观测化自动覆盖**：0.7 批③ 的预算 pill / 上下文组成体检对成员回合照常生效；侧栏频道行带生成中指示；轨迹页三事件 kind 归 CROSS 行（中文摘要）。
  - 侧栏改版：项目 scope 频道区块（活频道一行 + 未开启懒建入口）+ **搜索框整体移除**（无使用场景，用户拍板）。

### Fixed

- **频道接力风暴根治（四轮实测）**（生产实案：一条广播触发 8 跳接力链跑 10.5 分钟，末尾 3 次被链上限拦截；③ 批修复后成员已真做工具工作、共享流知识传播生效——本轮修的是接力调度层）：
  - **同目标重复入队 + 陈旧提及晚到出队**：统筹者 multi-@ 三名成员后，首个被唤醒成员的报告尾又把其中两名再 @ 一遍——队列无去重，同目标重复入队；FIFO 出队时第二次唤醒已是五分钟后，成员醒来「已交付完毕」白跑整轮，队列膨胀还把链顶到上限。接力入队现在按目标成员去重（已在队的目标不重复入队），唤醒时机改为最近 @ 者并刷新来源——共享流保证被唤醒成员一次读全流中全部指向它的 @，无需逐条各醒一次。顺带治渲染：通知条「X 点名 Y」的来源此前是入队时快照，出队呈现与相邻报告对不上，重复/晚到跳又成串出现——去重后不再有多余条目，来源刷新让提示与紧邻历史一致。
  - **`read_reference` 在频道是必败陷阱**：@ 引用钻取是 1v1 语义（引用快照只在 1v1 用户 @ 时物化，频道 @ 是纯寻址）——被唤醒成员困惑「没读到 @ 我的那段」时反复调它 hunting 原文、两连败。与 delegate 同闸收口：频道与委派子会话的回合不再注册该工具（共享流本身就是全量视野，无需钻取通道）。
  - **频道回合事件 actor 归因失真**：assistant 消息与工具执行事件的 actor 此前恒取会话行的 agent（频道里 = 统筹者投影），轨迹页看不出哪条是哪名成员的——改为执行成员本人（1v1 两者恒等，行为零变化）。
- **频道实测修复批（用户真机三反馈）**：
  - **无 @ 普通发送被显示成「用户 点名 X 接力」**：广播（统筹者接令）与真 @ 点名在事件里同用 from=用户 一个值，前端一律渲染成「点名」——用户每发一条消息都被误导以为自己 @ 过谁。事件增加广播语义位（旧事件零迁移），聊天区不再回显用户自己发起的派发（答案气泡已带身份头，回显即噪音），轨迹页按「广播 · X 接令」分词、完整事实保留。
  - **历史选举卡随新消息反复出现**：聊天区的频道事件交错渲染原给每个消息组挂「早于本组开始的全量事件单元」，一张历史选举卡被复制到它之后每一个消息组头上——每次发言前后都重复出选举卡（重复渲染键还会引发错位）。改为每个交错单元只挂到首个开始时间晚于它的消息组，历史事实在原位出现且仅一次。
  - **首条广播永久搁浅（选举触发错乱）**：统筹空缺 + 多条用户消息积压时触发选举，选举后的重消费头锚原取积压**末条**——只有最后一条被消费，首条广播从此无人应答。该路径未派发过任何消息，头锚改为回到积压**首条**（回归测试锁定）。
  - **选举投票行 sender 误归属 + 成员消息无身份**：频道内 agent 消息此前与 1v1 无区别（群聊看不出谁在说话），选举投票行的归属还会被回填兜底整批盖成统筹者。修法 = 频道占位行/投票行**创建即打 sender 标**（三出生点，sweep 兜底天然跳过已标行），前端两条 live 路径同步透出（回合开始权威刷新 + 流式占位继承上一条的 sender）——头像 + 昵称 + 统筹盾标全程在场。
  - **选举展示零散**：一届选举（开始 → 逐票 → 结果）此前以多条通知行 + 投票气泡散落消息流。现在聚成**一张选举卡**（标题 + 逐票行含弃权原因 + 当选结果/进行中态），按时间与消息交错；投票气泡不再重复出现。**聚合只发生在渲染前**——数据层 / 轨迹 / append-only 事件零改，与事件日志的契合保持原样。
- **频道不像群聊、更像只和统筹者聊**（生产实案取证：19 条用户广播全部由统筹者独答；被要求「问问其他人」时统筹者转用委派——四名成员在用户看不见的子会话里答完，频道只收到一张汇总表）：频道成员回合的提示词此前既无成员名册也无角色语境——统筹者不知道自己可以分派、不知道能 @ 谁。三种触发语境的事实简报（广播接令 / 用户点名 / 成员接力）现在全部注入成员名册（统筹者带标注）；广播接令臂明确统筹职责——更适合某位成员的问题就 @其名字分派（可多 @、被 @ 者在频道接力发言），不必事事亲自答；频道协作纪律同时披露「委派子会话对频道不可见，需要成员成果进入频道对话用 @」。委派工具在频道照常可用（当批口径：任务分发与频道接力正交，只做知情披露；下一批实测后翻转——频道不再注册委派，见下条）。
- **频道两轮对话全走委派、@ 接力被整体绕空**（生产实案：统筹者先发三路委派「探测」成员能否看到代码——委派子会话读不到频道历史、跑在隔离工作区，回传「看不到」的隔离答案固化统筹者错误信念，后续委派 args 原话「目前我们四位成员都说看不到项目代码」，用户手动终止）：根因不是模型不听话，是**委派在频道里是信息断流通道**——问了但拿不到共享流知识，答案也回不到流里、其他成员看不见。修复（均通用，不绑定任何具体项目配置）：频道回合不再注册 `delegate` 工具、可调度清单注入同步收口（防止提示词指向不存在的工具）；频道协作纪律换装**共享记忆句**——「频道历史是全体成员的共享记忆，其他成员在流中报告过的路径、发现与结论可直接取用」+ @ 分派附任务简述。路径知识传播由共享流结构承载（被 @ 成员读同一份频道全量历史；运行环境段恒披露当前工作目录与「工作目录外文件直接用绝对路径」；工作目录外的首次访问经一次批准后同目录本会话免再问）——**无需为项目配置工作区路径**。
- 频道首条消息静默失败（积压消费等待循环未启动）；侧栏统筹者误显（投影回退时把频道行 agent_id 当统筹者）。

## [0.7.0] — 2026-09-10

> 从 0.6.16 以来的主要调整：**MA-3 跨会话通讯**（0.7 主题——agent 向其它会话投递消息，同项目内异步对等通道）；**委派子会话降级链继承**；**上下文开销可观测化**（缓存 miss 归因 + 上下文组成体检）；MA-3 实测三轮修复 + 事件链路审计修复批 T。

### Added

- **委派子会话降级链继承（0.7 批 A 首件）**：专家 agent 没有自己的降级链时，自动继承父（统筹）agent 的降级链——委派任务不再因子 agent 未配链而在上游 404/429/额度耗尽时整单失败；子 agent 显式配链仍优先；继承链自动摘除与子 agent 主档相同的档（不重试已知失败）；父 agent 行读取失败降级为无链，绝不阻塞委派本身。
- **MA-3 跨会话通讯（0.7 批 B + 测试反馈批 + 二轮实测修复批）**：与委派互补的异步对等通道——agent 调 `send_message_to_session` 向其它会话投递消息，对方排队、空闲时消费一整回合；**边界 = 同一项目内的会话之间**（散落会话与跨项目投递执行期拦截）；`list_conversations` 系统级发现工具（查会话数量 / id / 所属 agent / 所属项目等概述信息，不看内容——可见性 ≠ 写权限）；收件三态（accept 自动（默认）/ hold 扣住待批准 / refuse 拒收；**expect_reply 的回投在 hold 下也免扣自动消费**）；收件箱 popover 条目双视图——自动消费语境（accept / 回投件）显示「排队中 · 会话空闲后自动处理」+ 次要「立即处理」钮，hold 语境保持「批准并消费」真审批（拒绝两步确认 + 政策切换），侧栏 badge 计数；来件以带来源标注的结构化消息落库（双块 + `incoming_source` 元数据，前端渲染不依赖文本格式）；`expect_reply` 回投链一次止，**双投递防重**——投递方开启自动回传时来件标注指引对方 agent 直接作答（不再叫它调投递工具），且消费回合内已手动投回则跳过自动回投（真机实案：双方每轮各收两条语义重复消息、多烧整回合上下文）；护栏：pending 上限 10、同会话 10 分钟窗口自动消费 ≤ 6 次。散落会话（未挂项目）隐藏收件箱入口——结构性收不到投递，不渲染永远空的死 UI；但有扣住的来件时仍显示（项目删除转散落后的处置出口，防死信）。pending 只入事件队列不写 messages——与目标在途回合零冲突。
- **上下文开销可观测化（0.7 批③）**：缓存为什么 miss、预算烧在哪，从此可见。
  - **预算 pill miss 归因 chip**：本轮输入全未命中缓存（≥ 1024 token 且命中为 0）时，预算 pill 旁出现灰字短标签（「疑缓存过期」「工具列表变化」「运行环境变化」「模型换档」等），hover 看全因机理；归因是本地推断，界面常驻披露。
  - **轨迹页「上下文组成」体检区**：turn 头概要新增本回合 prompt 组成分解——系统提示各段（人设/工具提示/运行环境/委派清单/样式档案）/ 工具定义 / 摘要 / 历史 / 用户消息各占多少 token（条形列表），估算合计与实际首轮输入的偏差行，逐轮命中序列（全未命中/工具列表变化/换档徽记）；新事件 `context_breakdown` 落库（构建期快照，进导出 JSONL）。
  - 门槛只在全 miss 时提示（部分未命中是常态）；provider 未回 usage 的回合诚实缺席不渲染。

### Fixed

- **MA-3 投递与事件日志链路审计修复批（0.7）**：四路扫描 + 生产库取证（52 检查项，防御性缺口 13 条入技术债台账批次 T 观察池），行为修复三件——
  - **Esc 关不掉收件箱浮层**：全局 Esc 关闭栈只认栈顶条目——「常驻注册 + 条件浮层」的空转回调（如删除确认条）会吞掉本该关浮层的 Esc。栈条目新增活跃谓词：浮层关着时让路，Esc 命中真正开着的层；全都不活跃则放行给组件自身（标题编辑输入框的取消编辑恢复直达）。
  - **生成中发第二条消息「没反应」**：在途回合（含正在观看的跨会话消费回合）期间按发送，此前静默吞掉输入；现在顶部横幅说明上一条仍在处理 + 一键重试，附件不丢失。
  - **跨会话来件消费回合不实时流入**：正在观看的会话收到来件开始消费时，来件卡（带来源标注）与回复此前的进入时机有误（要切走再切回才出现、回复看似凭空流出）；现在回合开始即权威刷新，生成中指示即刻可见。
  - 轨迹瀑布图收编跨会话来件两种事件（归入输入侧泳道 + 中文 tooltip，此前落模型道且显示原始事件名）。
- **收件箱浮层排版**：收件政策切换 segmented「自动接收（默认）」被等分三份挤成两行、提示与空态文案折行——改内容自适应宽度 + 恒单行文案契约（切档不再跳高度）。

### Changed

- 文档与代码对齐五处：事件词表实数 18 kind（文档原 14）、收件排空参数实值（2s 轮询·30s 上限）、收件政策 serde 默认对齐拍板值 accept（原残留 hold）、对账 skip 清单补全 `incomplete_turn_legacy_rows`、事件日志词表真相源指向本文件 typed emitter 集（migration 44 头注释仅记初始集）。

## [0.6.16] — 2026-09-09

> 0.6 线收官小版本：**全工程健康检查批 R**——五路扫描（后端不变式 / ModelProfile 线 / 错误路径并发 / 前端 / 文档对账）30 条入技术债台账，四批修复全推进（行为 / 健壮性 / 视觉 / 文档）。干净面结论：后端十条系统不变式 9 条成立、错误路径 P0/P1 零发现。

### Fixed

- **提案卡绕过 ModelProfile 引用语义**（行为修复首位）：引用态 agent 的配置提案若手填 provider/model/base_url，此前批准后下轮被 profile 解析静默覆盖——用户批准的换模型失效、设置页先显示「已改」再回翻、yaml 镜像还会把永不生效的值写进文件。后端守卫扩分支拒绝并三段式指路设置-模型页。
- **快速切会话消息串台**：会话加载的晚到响应不再覆盖新会话消息列表；「加载更早」不再把旧会话内容前插进新会话。
- **发送失败后幽灵「生成中」锁死输入**：失败横幅与回滚一律以发送时的会话为键（防切走后键值易主挂错会话），后台流式状态在失败路径正确清理——切回原会话不再停在假「生成中」。
- **keep-alive 缓存页后台空转两件**：设置-日志自动刷新定时器、项目任务事件监听——离开页面（缓存态）即停、返回恢复（listenerLive 门控）。
- **ModelProfile 解析失败静默回落旧 Key**：「配置行已删」（降级 legacy 合理）与「Stronghold 槽位损坏」（上抛报错）按失败步骤分流，不再一律静默换旧凭据继续跑、污染健康归因；链路测试对已删配置显式跳过。
- MCP server 开关切换失败静默假成功（内存态已翻、重启回退且无日志）；migration checksum 自愈虚报计数（失败也计成功）。
- 测试 mock 误编进生产二进制两处（补挂 `#[cfg(test)]`）；agent 建库后 KB 初始索引失败无日志。

### Changed

- 暗色主题错误横幅刺眼亮红改语义 token；附件卡/引用卡硬编码浅色收编；z-index/字号/间距零头令牌化（跳到底按钮/tooltip/会话头等）。
- 版本号三处文档性统一；CHANGELOG 补 0.6.4 与 0.6.15 档；CLAUDE.md 状态节与架构树对齐现状；CONTRIBUTING 死链与命令位置勘误。

### 真机测试点

- 引用态 agent 的配置提案 → 批准被拒并指路设置-模型页（不再静默失效）。
- 快速切会话 A→B：消息列表不串台；发送中切到 B 后 A 失败：横幅挂 A、切回 A 输入不被锁。
- 设置-日志开自动刷新后离开页面：后台不再每 5s 空转（DevTools 网络面板静止）。
- 暗色主题发送失败横幅观感（柔和 danger 底，非刺眼亮红）。

## [0.6.15] — 2026-09-09

> 从 0.6.10 以来的主要调整：**模型配置实体化（ModelProfile）全线**——Key 一处换全局生效 + agent 挂引用 + 降级链自动换档；**工具行 diff 徽记**；**启动白屏二轮根治**（WebView2 缓存瘦身）。中间 0.6.11~0.6.14 为测试包（装机验证未发布，版本号未使用）。

### Added

- **模型配置实体化（Phase 1）**：模型配置从各使用点散落抽成独立实体（设置-模型页管理，Key 收编 Stronghold 槽位）；视觉读取 / 语义检索改为引用实体——**Key 一处换全局生效**；健康三列监控（状态 / 最后错误 / 最后调用时间）；boot 存量明文 Key 自动抽离。通用页管引用、模型页折叠卡实体库。
- **agent 模型引用 + 降级链（Phase 2）**：agent 通过 `model_profile_id` 挂实体（四列快照制，值变才回写）；**降级链 `fallback_profile_ids` 有序多档**——主模型故障（额度耗尽 / 限流退避耗尽 / 网络不可达）按序自动换备用档，三拦截点接线、换档成功零错误终态、预算 cap 不重置；换档事件进轨迹与 toast；boot 存量 agent 配置抽离为实体（幂等标记，手动新建不误收编）。
- **新建自动物化 + 编辑链选择器（Phase 3）**：新建 agent 保留手写表单（零心智负担），保存时后端自动物化为实体（同配置去重复用）；编辑 agent 改为合并 tag 链选择器——主+降级同框、首 tag 主档徽标、拖拽排序 + 键盘换位、每档健康点、候选排除已选、「+ 新建模型配置」跳设置页。
- **链路仿真测试**：agent 编辑页一键测试整条降级链——逐档真发一条 16 token 摘要请求（列模型是鉴权层动作验不出对话权益），换档判据与运行时同一张分类表，逐档沉淀健康状态。
- **工具行 diff 徽记**：edit_file / write_file 工具行展示 git 式 `+N -M ~K` 三计数（绿/红/黄语义色）；**存量回填**——0.6.14 前的 edit_file 历史记录从参数与替换计数重算，旧会话也出徽记。
- **会话头 agent 名点转**：点击跳设置-智能体页对应编辑卡（展开 + 滚入视野，含 keep-alive 二次进入路径）。

### Changed

- **通用设置页三卡迁移显式保存**（编辑交互契约统一）：草稿态 + 保存/取消，撤选择即存。
- 设置页内容列统一限宽居中（消除通用页 780px 与其余全宽的切页割裂）。
- 思考耗时单位小写（`1m 30s`，CSS uppercase 根治）。
- 全仓 cargo fmt 规整（工作区曾经历非标准紧凑化格式，84 文件零语义变化归位）。

### Fixed

- **启动白屏二轮根治**：真凶为 WebView2 用户数据目录缓存膨胀（实测 606MB）——首帧骨架结构性覆盖不到 index.html 之前的加载段。三件：版本升级时清缓存 + `--disk-cache-size` 50MB 封顶；主窗 `visible:false` + 前端挂载后 show + 后端 10s 兜底（窗口迟到但出现即完整，消灭「白屏窗悬着未响应」）；boot 锚点日志（setup 前段 / 总耗时）。
- **测试连接健康归因**：存了自定义端点 / Key 的档位点「测试」状态点与最后调用时间不更新——前端把未改动回显值当覆盖值传 + 后端归因条件过严，双端修复（入参与存量等值降级为不覆盖，恢复归因）。
- 模型页引用计数漏算 agent 腿（主档 + 降级链）。
- 编辑态引用形态头像压扁（宽度钉死 160 而行高缩短——引用态 120px 正方）。
- 链选择器开合点击区被拖拽根元素吞掉（只剩 chevron 可点）。

### 真机测试点

- 白屏五判据：首启清缓存稍慢属预期 / 窗口迟到即完整 / EBWebView Cache 重建 ≤50MB 量级 / 日志两条 boot 锚点 / 审批 toast 前置与窗口状态记忆回归。
- edit_file 历史行徽记回填（write_file 历史行无徽记属诚实边界）。
- ModelProfile 主链：新建物化 / 换 Key 全局生效 / 链选择器交互 / 链路仿真 / 降级链真发 429 造换档（详见台账批次 V·手测）。

## [0.6.10] — 2026-09-07

> 从 0.6.4 以来的主要调整：**轨迹页事件类型筛选**（仅对话预设 + 类型多选下拉）、**工具行级摘要**、**发送热路径性能三减**、启动白屏 / run_command 引号 / 探测链三修。中间 0.6.8/0.6.9 为测试包（装机验证未发布，0.6.5-0.6.7 版本号未使用）。

### Added

- **轨迹页事件类型筛选两件**：「仅对话」一键预设（只留用户消息与回复，轮次骨架与统计保留）+「类型」多选下拉（4 组 10 键细筛，带窗口计数徽标与恢复默认）——两者操纵同一筛选状态，偏离预设即药丸自动熄灭；筛选偏好持久化且跨会话保留，项目时间线与轨迹页共享。原「辅助事件」布尔开关由细分取代。
- **「加载更早」显示逻辑**：滚到已载窗口顶部（或加载中）才淡入，常驻 DOM 开合无布局跳动；加载补偿后自动淡出。
- **工具行级摘要**：对话页工具调用气泡收为单行形态，思考耗时持久显示，委派卡精确绑回。
- **工具展示名中文化**：设置-工具集主名中文（扩至 40 条），原名降次位显示；工具集页支持搜索与分组。
- **运行态图标双形态**：工具调用行 spinner、其余状态九宫格像素格。

### Changed

- **发送热路径性能**：每轮 O(N) 三减（对账后台化 / supersede 哈希 / tail-limit 前置），千轮会话发送路径 bench 1562ms→350ms。
- 探测（测试连接）超时放宽至 10/20s，网络错误文案三段式，`base_url` 双层 Option 反序列化修正。

### Fixed

- **已删项目页死态常驻**（生产实案）：项目被永久删除后，重进该项目设置页顶部常留「项目不存在或已删除。」且无法恢复——根因是外层 keep-alive 以组件类型缓存、缓存实例复活时挂载兜底不再跑，加上启动恢复对路由裸放行（会话/范围都校验了，唯独路由）。三件：布局兜底接缓存复活/跨项目导航/挂载后被删三条路径（死项目必得「找不到该项目。」+ 返回列表可恢复态）；设置 tab 死端补返回按钮；启动恢复拦截已删项目路由降级首页（归档项目仍可直链）。
- **启动白屏**：孤儿 tool_result 扫尾误匹配死循环治本 + 启动重活（清扫/回填）后台化 + index.html 首帧骨架。
- **run_command Windows 引号**：`raw_arg` 直通道（含引号/特殊字符的命令不再被拆碎）+ 子进程随会话终止 + 回显防御。
- 轨迹表思考/续写图标独占一行（双行）问题根治。
- 工具层错误契约补课七件（含 search_files 静默失败治本）。
- 气泡内运行态图标回归（spinner 撤除回九宫格）。

### 真机测试点

- 轨迹筛选：仅对话一键 / 类型下拉勾选与恢复默认 / 持久化跨会话与项目时间线共享 / 加载更早顶部淡入、加载后淡出不跳。
- 发送性能：千轮级会话发送与续写响应明显变快。
- 白屏四判据、run_command 四判据、探测超时文案（详见 0.6.8 批台账）。



## [0.6.4] — 2026-09-04

> 从 0.6.3 以来的主要调整：**第三轮自查六路扫描质检批 Q 四组**、**委派审批改造**（会话级授权记忆 + 委托预授权）、**审批通知直操作**。

### Added

- **委派审批改造**：L0 授权记忆会话级（本会话批过「此工具」后续轮次不再弹）+ L2 委托时预授权两档（子会话继承授权口径，「命令免问」档子会话 run_command 直接跑）；拒绝文案回传父 agent。
- **审批通知直操作**：Windows toast 自带批准/拒绝按钮（免回窗）；点通知主体前置主窗；single-instance 防双开；失焦通知恰一次。

### Fixed

- **copy/move 双路径越界授权**：原实现只校验 source 在工作区内即静默放行——destination 可越出授权面写任意路径。改双路径 all-match（destination 越界弹 Confirm）。
- **doom 错误签名被变体前缀稀释**：Validation Display 前缀把同变体所有家族折叠成一签名（混家族 6 连败误终止）。签名先剥已知变体前缀再截家族词。
- 外部 MCP tools/call 超时不清 pending 表（挂死 server 上无界小泄漏）。
- event_bus 转发任务遇 broadcast Lagged 永久退出（`session:event-appended` 全局停发到重启）。
- **多轮工具回合冻结轮瞬态消失**：上一轮工具记录在下一轮 TTFT 期间消失、done 后恢复——骨架门控从全局 `sending` 改 item 级 `isLiveAssistant`。
- CI Linux cfg 分支修复。

### Changed

- **视觉规范批**：语义色圆点取代 emoji 档位标识；轨迹 / KB / 审批卡图标收编 Lucide；tokens.css 自动暗区镜像补齐（success/warning/danger/info 全族）；间距令牌化；TrajectoryView keep-alive 监听成对（缓存态不再后台空转）。
- **性能批**：附件物化 / KB 索引器 / docx zip 手术三处同步重活 `spawn_blocking`（治生成期 UI 卡顿同病灶）；KB 语义检索四标量签名向量缓存（不再每次全量加载 chunk，rowid 回收陷阱以第四标量兜住）。

### 真机测试点

- 委派审批五点（L0 跨轮免弹 / 委派卡两档 / 命令免问档 / 拒绝回父 / 后台栈允许=逐次档）。
- 通知六点（失焦恰一次 / toast 直操作 / 前置主窗 / 双开防护）。
- copy/move 越界应弹 Confirm；KB 二次检索走暖路径；Word 生成期 UI 不卡。

## [0.6.3] — 2026-09-01

> 从 0.6.2 以来的主要调整：**侧边栏收起 rail 模式**、**底缘渐隐与置底吸附联动**（对话/轨迹两 tab）。

### Added

- **侧边栏收起/展开（rail 模式）**：header 收起钮一键折为 56px 单列行动栏——展开钮 / 新建对话 / 会话列表 flyout / 项目空间 flyout / 设置。会话列表压缩为单入口 + flyout 菜单（不展开侧栏），头部搜索与展开态同款、共用过滤逻辑；项目空间收起为图标钮，菜单向右弹且头部收进「快速新建（表单菜单内原地展开）/ 项目列表」两入口。收起状态本地记忆、展开还原用户上次拖的宽度；宽度过渡 300ms 且只在动画窗口生效（不干扰拖把手调宽）；reduced-motion 下瞬时切换。
- 主题切换钮自侧栏 header 迁至底部（与设置同行右对齐），圆形扩散动画起点自动跟随新位置。

### Changed

- 消息区/轨迹表底缘渐隐与「置底吸附」联动：滚动条贴底时渐隐蒙层撤下（内容完整可读），离开底部渐隐恢复；渐隐区 72→96px，消息区底 padding 32→16px 更贴近输入框。

### 真机测试点

- 收起/展开过渡平滑、拖拽调宽无橡皮筋、重启恢复收起态无宽度闪烁；
- 会话 flyout：搜索过滤 / Esc / 点外关 / 选中跳转；项目菜单头部两入口 + 菜单内快速新建表单；
- 暗色模式视觉、收起态下最小窗口（900×600）flyout 不触底。

## [0.6.2] — 2026-09-01

> 从 0.6.1 以来的主要调整：**Word 十波 TOC 目录 + 图片插入（D18）**、**十一波生产坑三件抽象化（D19）**、**enabled_tools 旧白名单复活根治**（0.6.1 装机生产实案）、**生成期切项目详情卡顿三修**。

### Added

- **Word 十波——TOC 目录 + 图片插入（D18）**：`write_docx` 新块型 `toc`（levels 1-9 缺省 3 / hyperlink 缺省 true）与 `image`（path 挂 workspace 相对或绝对、png/jpg ≤10MiB、width_mm 钳版心不放大）——一次成文即可带目录与配图；`edit_docx` 新操作 `insert_toc_after` / `insert_image_after`（图片走 template 先例的读侧第二路径）；`validate_docx` 断言 `block_image` / `block_field`；`inspect_docx` 段尾 `[图片×N]` `[域:TOC]` 标记（治「插完图读回来是空段」盲区）。TOC 为裸 fldSimple + updateFields——Word 打开自动生成目录（WPS 不保证，自愈文案指路 F9）；包级手术 rels/CT/settings **只增补绝不重编号**（重编号 = Word 拒载）。
- **Word 十一波——生产坑三件抽象化（D19）**：①「占位段不是内容」——横并/聚合不再搬运无文字/图/域的空段（治段落膨胀），摘要披露丢弃数；②「重写保真到位置对齐粒度」——单元格重写段数对齐时逐位继承 pPr/rPr（治重写丢格式要多轮补批）；③表内地址模拟器——结构操作（merge/split/delete_table_row）声明行区间足迹，**不相交即可同批**（整列 13 次横并 → 1 次调用），相交拒批点名冲突对，取代「结构重构独占一批」类型黑名单。

### Fixed

- **enabled_tools 旧白名单复活根治**（0.6.1 装机生产实案：升级激活旧白名单、摘 yaml 键行后 docx 工具静默缺失）：`set_agent_enabled_tools` 摘除/收窄后镜像同步 DB 列（组装期收窄读 DB 行而非 yaml），组装收窄生效时打披露日志「保留 N / 裁掉 M + 名单」；生产 workaround（yaml 手加 `enabled_tools: []`）可摘。
- **生成期切项目详情卡顿三修**：① 后台流式快照每 40ms 整替 Map，致侧栏/项目列表/任务台账等 8 处订阅无谓重算（改原地改写）；② 项目轨迹事件监听器随路由级 keep-alive 缓存泄漏——看过一次轨迹 tab 后，任何页面上的事件落库都驱动「查询+重算+重渲染」后台空转（离开暂停/回来恢复 + 300ms 合并）；③ 项目轨迹尾页 SQL 对全部项目事件含载荷排序后才截断（先取一页 id 再回表，生产库 33ms→12ms 随项目规模线性劣化消除）。
- 会话页输入区与消息区分隔改底缘渐隐（撤输入区上边线，72px 三停渐变带）。

### 真机测试点

- write_docx 带 toc+image 块生成报告：Word 打开目录自动生成、图片按版心宽显示；WPS 打开看 TOC 是否自动刷（不刷则 F9 自愈文案可见）；
- 表内地址模拟器：整列横并一次调用成功 + 足迹相交拒批文案；
- 卡顿三修：生成/改 Word 中切项目详情，点击响应与消耗图加载是否恢复正常（若仍复现，抓主线程 longtask 定位）。
...
- **Computer Use —— 看屏**（GDI 捕获；坐标契约 = 本会话最近一次截图的像素空间，输入前三步换算 + 显示器布局 revalidate）：
  - `capture_screen` 整屏/区域截图；`capture_window` PrintWindow 免聚焦窗口捕获；`list_windows` 窗口清单（EnumWindows 五条过滤）；
  - 全屏识别启发式：帧均匀性分级（等步长采样）——连续 2 次整屏捕获纯色 = 目标可能全屏独占（游戏）或 DRM 保护内容，家族错误指路 `capture_window`；≤4 色 warning；前景窗口矩形包住整显示器 → 提示 note；
  - 截图历史压缩双钩：in-flight 上下文只留最近 3 张图（loop 轮内钩 + 跨回合 Stage），DB 与会话事件日志保完整图——token 洪峰结构性兜底。
- **Computer Use —— 操作**（SendInput 原语；截屏/鼠标/键盘全 Confirm 级授权 + `auth_reason` 披露；`list_windows`/`wait` Always 级）：
  - 鼠标四件 `mouse_move`/`mouse_click`/`mouse_drag`/`mouse_scroll` + 键盘三件 `type_text`（KEYEVENTF_UNICODE 逐 UTF-16 码元，绕键盘布局）/`press_key`（修饰键组合，按下→点按→逆序释放）+ 节奏件 `wait`（50..=10000ms 钳制，可被取消打断）；
  - **act-and-look**：六件操作成功后自动附「操作效果图」——等画面稳定（隔 250ms 两帧逐字节相同才收敛，上限 2s；被动变化场景附最终态）→ 附进同一 tool_result → **即刻写为新坐标基准**（结构性消除过期坐标 + 省一整轮确认往返）；
  - **人类优先仲裁**：物理鼠标/键盘输入（WH_*_LL 钩子只登记非注入事件——SendInput 自己不算人类在场）2s 去抖判定「用户在场」→ agent 写操作让路（读不受影响），闲置自动恢复；序列操作逐边界检查点（拖拽每步/逐字符/逐轮按键），抢占即安全收尾（释放按住按钮 + 报已完成进度）；
  - **单写者仲裁**：通道令牌 + 排队（多会话写操作串行化）+ 暂停 gate（读写分级，读不受影响）；排队情报附在写结果里对模型可见；各状态错误文案走稳定家族前缀（doom_loop 连败检测依赖）。
- **屏幕共享通道**（授权与可见性的单位，治「移动/点击每步弹审批卡打断操作」）：
  - 会话头「屏幕共享」开关 = 唯一入口：开启一次 Confirm，此后屏幕家族工具免逐次审批卡（授权短路）；关闭即全部停止（通道状态不跨重启持久化）；也可对话内让 agent 调 `request_screen_session` 发起；
  - **HUD 工具栏窗 + 红边框**：置顶 HUD 显示共享时长/令牌持有者与 purpose/队列与授予/「你在操作」冲突胶囊/截图计数/显示器切换/暂停/终止；全虚拟桌面 3px danger 红边框（点击穿透）=「屏幕正在被共享」持续可见信号；agent 写操作期间 HUD 自动收缩让路、结束自动恢复；会话内仅保留屏幕共享开关一个按钮（暂停/终止住 HUD）。
- **视觉读取两档制**（取代旧三环借凭据）：agent 自身有视觉 → 走自己；无视觉 → 走「设置-通用-视觉读取」平台配置条目链（主模型 + 可选降级按序尝试，首个成功即用；未配置 → 剥离图 + 诚实提示指路设置页）；代读 marker 披露实际读图模型；`test_vision_config` 逐条健康检查（1×1 PNG 探针全链路，测过=正式可用）；前端条目卡编辑器（厂商/模型/Key/端点 + 逐条测试）；GLM 视觉 MCP 模板撤下（Coding 套餐专属，借标准端点必 1113 的假环）。
- **Agent 配置一致性两批**：
  - 旋钮唯一写入通道（治「批准后生效」假象，最痛件）：system_prompt/temperature/max_tokens/enabled_tools 等旋钮字段的写入通道 = yaml 通道命令（`set_agent_yaml_field` 扩浮点+整数族、新增 `set_agent_enabled_tools`），`update_agent` 只管出生证字段；提案批准按字段分派，用户编辑值覆盖优先；
  - `base_url` absent=保持语义（显式给=设/清；不带 base_url 的 update 不再静默清空端点，仅换厂商且有默认时跟随）；`enabled_tools` 死配置转真（组装期非空名单收窄注册表快照，平台元工具恒保留）；流中模型 staleness（回合进行中显示发送时快照）；has_api_key 真相实查 +「去申请 Key」直达；PROVIDERS 目录补 claude-opus-5/sonnet-5/fable-5/haiku-4-5。
- **首用引导**：错误横幅行动化（LLM 错误细类 →「去检查配置」直达）；无可用 Key 警示条；侧栏空态细分引导；种子帮助文档对齐现状（存量装机删文件重启自动补回）。
- **新模型跟进**（官方文档核实 2026-08-27）：GLM-5.3（纯文本旗舰，[1m] 后缀 1M 窗口）、GLM-5.3-Flash（GLM-5 系首个原生多模态，官方定位 GUI agent 对口）、DeepSeek 视觉实验模型（deepseek-v4-flash-vision-exp）——窗口表/厂商目录/视觉建议表三表刷新；DeepSeek 换 v4 系新名（chat/reasoner 旧名已弃用）。
- **通用设置页卡片分组重设计**：三卡信息架构（本地环境/语义检索·知识库索引/视觉读取·图片代读）+ 保存反馈统一（工作空间选定即存、按字段粒度成功提示）+ 瞬态测试提示切页即清 + 语义检索区新增「测试」按钮（与视觉区对称）。

### Fixed

- 摘要通道 `thinking:disabled` 与 GLM-5.3 系不兼容（5.3 全系拒 disabled，硬发请求必败）——改注入 enabled + reasoning_effort=low；utility thinking 三态唯一真相源，摘要与图片代读共用；
- 轨迹页暗色主题下选中行近白底浅字不可读（selection 令牌误用于行选中）；
- 视觉读取编辑器系列：测试按钮参数 camelCase 对齐命令层 / 唯一条目可删 / 新增条目即落库 / Coding 端点分家（1113 同坑二修）/ 条目卡布局与光标错域。

### 真机测试点

- Computer Use 全链路：屏幕共享开关 → 授权卡开启 → agent 截图并描述桌面 → 移动/点击/输入/拖拽 → HUD 暂停/终止/写避让收缩体感/显示器 ◀▶；人类优先（agent 操作中动一下鼠标 → 让路）；act-and-look 连贯性（操作后无需再手动截图确认）；
- 视觉两档制：非视觉 agent 发图 → 代读 marker 带实际读图模型名；设置页逐条「测试」；降级链第二条目接管；
- 批次③细项：中文输入（KEYEVENTF_UNICODE 路径）/组合键/滚轮/wait 中断；多显示器（换硬件）。

## [0.5.5] — 2026-08-26

> 从 0.5.4 以来的主要调整：**换厂商配置分裂根治** + **智谱 Coding 端点显式切换与 1113 指路**（两条生产实案）。

### Fixed

- **换厂商配置分裂**（生产反馈）：agent 换 provider 后 UI 显示新厂商而 agent.yaml 镜像停在出生值，端点/Key 不联动（旧厂商 Key 打新端点 → 报对端厂商错误）。三件：update() 同步 yaml 的 provider/model/base_url 镜像行（文件存在才补丁、原子写、best-effort）；换厂商且未显式给 base_url → 端点重置注册表默认（防 DB 残留旧厂商 URL）；前端换厂商必填新 Key（免鉴权 ollama/custom 放行）。
- **智谱 Coding 端点 1113**（生产实案）：Coding 套餐 Key 打标准端点报 1113「余额不足或无可用资源包」——套餐有余额仍报（Coding 额度只在 Coding 端点生效，两套端点 Key 不通用；「测试连接」列模型是鉴权层动作，标准端点对 Coding Key 假绿固化错误端点）。三件：AgentForm URL 框下端点胶囊显式切换（带 alt_urls 的厂商才渲染，当前仅智谱；测试连接显式传所选端点）；错误分类细分 GlmResourcePack（措辞含「无可用资源包」须先于 429/余额通用分支）文案三段式指路端点切换；注册表 note 更新。

## [0.5.4] — 2026-08-26

> 从 0.5.3 以来的主要调整：**S3 七波·删行+批组合三放宽**（生产 agent 缺口报告第二弹，五条对码核实分流——两条实为「引擎已支持、描述不清」）。

### Added

- `delete_table_row`（P0 真缺位——agent 曾被逼用 trPr hidden 藏行）：纵向合并三态守卫——行内含合并头且下方同网格列有续格 → 拒并指路 split_cell；纯续格行可删（合并链条缩短仍合法）；仅剩 1 行空表保护指路 delete_block。结构重构独占一批。
- 同格多元素同批：`set_table_element` 按元素去重（如 vAlign + tcBorders 一批序无关）；内容/格式手术保持每格限一条（重写语义互斥）。批组合判据：序无关/序确定才可组合。
- 同锚多段链式：同锚块多条 `insert_paragraph_after` 按输入序聚合进单个插入 splice（镜像表格路径的 plan 索引聚合，不做偏移数学——块间缝隙可为 0）。
- description 澄清两处非缺口：跨表格块本就可同批（全文档统一边框一句话搞定）+ 列宽按内容加权 tcW 配方。
