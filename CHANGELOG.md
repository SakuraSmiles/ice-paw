...
- **频道标题头部重设计**：标题回归项目名本体（存量「 · 频道」后缀由 migration 剥除），频道身份改由紧跟标题的 tag 徽章呈现（无 # 图标；归档频道显示警示色「已归档」）；子标题改为成员头像叠层（前 4 枚 + 溢出「+N」）+「N 名成员」+ 统筹者标注（待选举灰字；不显示统筹者的模型——广播路由下每位成员都可发言）；右侧统筹者胶囊换成成员图标方钮（名字已进子标题不重复占位，点开同一个频道成员浮层）；归档频道子标题显示「已归档 · 记录只读」。
