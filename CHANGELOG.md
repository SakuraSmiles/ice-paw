...
- **MA-3 跨会话通讯（0.7 批 B + 测试反馈批）**：与委派互补的异步对等通道——agent 调 `send_message_to_session` 向其它会话投递消息，对方排队、空闲时消费一整回合；**边界 = 同一项目内的会话之间**（散落会话与跨项目投递执行期拦截）；`list_conversations` 系统级发现工具（查会话数量 / id / 所属 agent / 所属项目等概述信息，不看内容——可见性 ≠ 写权限）；收件三态（accept 自动（默认）/ hold 扣住待批准 / refuse 拒收；**expect_reply 的回投在 hold 下也免扣自动消费**）；hold 来件走会话头收件箱 popover（批准/拒绝两步确认 + 政策切换），侧栏 badge 计数；来件以带来源标注的结构化消息落库（双块 + `incoming_source` 元数据，前端渲染不依赖文本格式）；`expect_reply` 回投链一次止；护栏：pending 上限 10、同会话 10 分钟窗口自动消费 ≤ 6 次。pending 只入事件队列不写 messages——与目标在途回合零冲突。
