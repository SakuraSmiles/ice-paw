...
describe("InboxPopover", () => {
  ...
  it("渲染权威列表：条目源信息 + 计数 + 期待回复标记", async () => {
    ...
    expect(w.findAll(".inbox-item")).toHaveLength(2);
    expect(w.text()).toContain("2 条待处理");
    expect(w.text()).toContain("甲 · 来自「主控」");
    // expect_reply 条目带标记，非期待条目不带
    expect(w.findAll(".inbox-reply-flag")).toHaveLength(1);
  });
  ...
  it("政策切换：乐观切 + setPolicy；失败回滚 + 错误横幅", async () => {
    ...
    // 成功路径：切自动接收
    await btnOf("自动接收").trigger("click");
    await new Promise((r) => setTimeout(r, 0));
    expect(mockSetPolicy).toHaveBeenCalledWith("c1", "accept");
    expect(btnOf("自动接收").classes()).toContain("active");
