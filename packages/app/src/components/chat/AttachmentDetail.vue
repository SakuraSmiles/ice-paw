        ...
        <div class="att-detail-list">
          <div v-for="(att, i) in attachments" :key="i" class="att-detail-item" :class="{ open: expanded[i] }">
            <button class="att-detail-toggle" @click="toggle(i)">
              ...
              <ChevronDown v-if="expanded[i]" :size="14" class="att-detail-arrow" aria-hidden="true" />
              <ChevronRight v-else :size="14" class="att-detail-arrow" aria-hidden="true" />
            </button>
...
.att-detail-title { font-size: var(--ip-text-body-size); font-weight: 600; color: var(--ip-color-text-primary); }
.att-detail-close {
...
.att-detail-list { overflow-y: auto; padding: var(--ip-spacing-2); }
...
.att-detail-toggle {
  ...
  padding: var(--ip-spacing-2_5) var(--ip-spacing-3); border: none; background: transparent;
...
.att-detail-icon {
  ...
  display: flex; align-items: center; justify-content: center;
  font-size: var(--ip-text-body-sm-size); font-weight: 700; color: var(--ip-color-text-on-primary);
}
...
.att-detail-name {
  font-size: var(--ip-text-body-sm-size); font-weight: 500; color: var(--ip-color-text-primary);
  overflow: hidden; text-overflow: ellipsis; white-space: nowrap;
}
.att-detail-meta { font-size: var(--ip-text-micro-size); color: var(--ip-color-text-tertiary); }
.att-detail-arrow { flex: none; color: var(--ip-color-text-tertiary); }

.att-detail-text {
  padding: 0 var(--ip-spacing-3) var(--ip-spacing-3) 56px;
}
.att-detail-text pre {
  margin: 0; padding: var(--ip-spacing-2_5) var(--ip-spacing-3);
  background: var(--ip-color-bg-tertiary); border-radius: 6px;
  font-family: var(--ip-font-mono, ui-monospace, monospace);
  font-size: var(--ip-text-caption-size); line-height: 1.5;
