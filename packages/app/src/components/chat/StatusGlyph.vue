<!--
  ...
  五态（色走语义层 + 主色，勿再加新色）：
  - running：variant="pixel"（默认）= 3×3 像素格顺时针点亮循环（视频签名元素）；
             variant="spinner" = 单线渐变环旋转（聊天气泡内进行中——用户拍板
             2026-09-05：气泡内转圈、其余位置保持像素格）。主色，进行中是信息态非警告态
  ...
  3. 循环时长唯一来源 --ip-duration-pixel / --ip-duration-spinner（tokens.css），勿散点硬编码。
  4. 状态切换重播：v-if/v-else-if 分支切换天然重挂载（mount 动画自动重放），勿改成
     单元素 class 切换。
  5. 气泡内 glyph 都在 flex 居中行（tool-toggle/think-toggle）——StatusGlyph 根是
     无文本 inline-flex，勿放进非 flex 的 inline 文本流（baseline=底边会整体下坠，
     ChatHeader 徽标 2026-09-05 实案）。
-->
...
withDefaults(
  defineProps<{
    ...
    label?: string;
    /** running 形态：pixel=九宫格（默认，TaskPanel/委派卡/会话头）；spinner=单线
     *  渐变环旋转（聊天气泡内的思考/工具/生成指示）。非 running 态无差。 */
...
/* ----- running · pixel：3×3 像素格（CSS Grid，循环点亮） ----- */
.glyph-grid {
  display: grid;
...
.glyph-error {
  position: relative;
  border: 1.5px solid var(--ip-danger-base);
  color: var(--ip-danger-base);
}
/* X 绝对定位居中（2026-09-05：X 对称形状对偏移最敏感——flex 居中在个别容器里
   受行内环境影响有亚像素偏移，绝对定位 + translate 是无条件正中）。X 缩一号
   （size-6）+ stroke 加粗到 ≈环宽：旧值 9px/1.1px 在 11px 内径里贴环，
   视觉上「叉压着环/不在中心」。 */
.glyph-error > svg {
  position: absolute;
  top: 50%;
