<!--
  ...
  - turn-header 40px：整行带（浅底、去 rail 去圆角；左缘内缩 8px，比子项贴边——
    ...
    点击折叠/展开；折叠态 = 只留头；频道接力/选举归父进轮内（整条链一个头）；
    轮外段（跨会话来件/统筹位变更）标签特殊化、不占轮号、无终止徽与回复统计
...
/** turn 头轮次标签：轮外段（跨会话来件/统筹位变更）不是对话轮——标签特殊化
 *  不占号；频道接力/选举已归父进轮内（⑯ logicalTurnKey）成为轮内 CROSS 行，
 *  整条链一个头一个轮号 */
    ...
    <div class="ttab-canvas" :style="{ height: `${viewport.total}px` }">
      ...
      >
        ...
        <template v-if="item.row.type === 'turn-header'">
          ...
          <!-- 轮外段无 turn_ended——不渲染假「进行中」；归父后的频道链折进父轮终止徽 -->
...
/* 轮外段标签（跨会话来件/统筹位变更）：非对话轮，弱化以别于真实轮号 */
