...
> 保存时，这套厂商+模型+端点+Key 会自动收编成一条**模型配置实体**（相同配置只建一条），之后换 Key、配备用模型降级都在**设置 → 模型**统一管理，一处换全局生效。编辑 agent 时只能从已有实体里选。详见「模型配置与降级链」。
...
配好基础对话后，可以接着开：
- **模型配置与降级链**：Key 管理、备用模型自动换档 → 见「模型配置与降级链」。
- **图片与视觉**：发图给 agent 看、非视觉模型代读 → 见「图片与视觉能力」。
- **知识库（KB）**：让 agent 检索你的文档 → 见「配置知识库与 embedding」。
- **工具（MCP）**：让 agent 读写文件、跑命令、接外部 server → 见「配置 MCP 工具与授权」。
- **Word 文档**：读、改、整篇生成 .docx → 见「Word 文档能力」。
- **屏幕读写**：让 agent 看屏幕、操作电脑 → 见「屏幕读写与共享」。
- **多 agent**：委派、跨会话投递、项目频道群聊 → 见「多 agent 协作」「频道：项目成员协作」。
