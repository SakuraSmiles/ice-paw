...
//!
//! **W1.2 起**：跨层共享的类型（`ChatMessage` / `ContentBlock` / `ChatDelta` /
//! `ToolDef` / `TokenUsage` / `LlmProvider` trait）已迁移至 `crate::infra::protocol`。
//! 本模块只保留：
//!   - 子模块（adapters / cancel / chat_state / tool_registry）
//!   - `create_provider()` 工厂
//!   - `default_base_url()` 辅助函数
//!   - 公共 re-exports（CancellationToken / ChatState / ToolRegistry）
//!
//! P2-1 升级历史：
