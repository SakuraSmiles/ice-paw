...
// ============================================================================
// U3-2 ①：以下类型 / 函数已下沉至 `harness::fallback_plan`（破除 harness 反向
// 依赖 commands 的层级倒置）。本文件经 re-export 保留命令层原调用路径——
// agent_cmd / chat_cmd / provider_cmd 的 `crate::commands::model_profile_cmd::*`
// 路径零改动；channel / inbox / delegate 等 harness 调用方改走 harness 直连。
// ============================================================================

pub use crate::harness::fallback_plan::ModelProfileWithCredentials;
pub(crate) use crate::harness::fallback_plan::{
    production_fallback_plan, resolve_profile_credentials, ResolveProfileError,
};
