...
pub(crate) const CHANNEL_HINT: &str = "你正在项目频道的共享对话流中与多位成员协作：你的回复对全体成员可见；需要某位成员接手时，在回复中 @对方名字（其回答会接力出现在频道里）；委派（delegate）的子会话对频道不可见——需要成员成果进入频道对话时用 @ 而非委派；动手前先看流中已有的进展，不重复他人已完成的工作；对分工有疑问时，先说明你的理解再行动。";
