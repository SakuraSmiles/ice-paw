...
///
/// 2026-09-16 增两条交流经济纪律（聊天区过程收纳配套的源头治理，用户拍板
/// 「组合拳」）：过程叙述克制（治「偏多偏杂」源头——生产实案 50 轮组 21 段
/// 正文多为模型重复计划式碎句）+ 终答自包含（治末段质量——生产库实测大组
/// 仅 50% 末段完整收尾；截断回合提示词治不了，展示侧以「回合被截断」标注
/// 兜底）。措辞避开风格预设的「先给结论」（两层边界测试锁）。
...
- 多轮工具调用的过程中，每轮的意图说明保持一句话简洁，不重复已声明的计划。\n\
- 任务完成时，最终回答先用简短总结说明做了什么与结论，再给必要的细节。\n\
- 使用用户所用的语言回复。";

/// 频道协作纪律（channel v1，仅 kind='channel' 会话注入）。
///
/// 平台层口径（风格中立）：只说清频道与 1v1 的**结构差异**——多人共享流、
/// @ 交接、共享记忆、不重复劳动；不说协作风格（那是人格）。短一段刻意不
/// 超载：详细协作礼仪由成员 agent 自己的人格与事实简报承载。
pub(crate) const CHANNEL_HINT: &str = "你正在项目频道的共享对话流中与多位成员协作：你的回复对全体成员可见；需要某位成员接手时，在回复中 @对方名字并简述任务（其回答会接力出现在频道里）；频道历史是全体成员的共享记忆——其他成员在流中报告过的路径、发现与结论可直接取用；动手前先看流中已有的进展，不重复他人已完成的工作；对分工有疑问时，先说明你的理解再行动。";
...
mod tests {
    ...
    /// 平台层纪律锚（2026-08-23 两层设计）：叠加生效声明 + 角色无关纪律。
    /// 意图确认**不在**平台层（与创作预设第一条重复，下沉工程档）。
    /// 2026-09-16 增两条交流经济纪律锚（终答自包含 + 过程叙述克制）。
    #[test]
    fn system_prompt_platform_disciplines_present() {
        ...
        assert!(result.contains("不重复已声明的计划"));
        assert!(result.contains("简短总结说明做了什么与结论"));
