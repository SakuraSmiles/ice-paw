...
use super::{
    inspect_document, try_extract, try_extract_chunks, DocKind, InspectProjection, InspectRequest,
};
...
fn corpus(name: &str) -> Option<Vec<u8>> {
    ...
    match std::fs::read(&path) {
        ...
        Err(_) => {
            eprintln!(
                "skip：docx 语料 {name} 不存在（敏感文件不入库，仅本地 {}）",
                path.display()
            );
            None
        }
...
fn corpus_sdp_full_extract() {
    let Some(sdp) = corpus("sdp.docx") else {
        return;
    };
    let doc = try_extract(&sdp, "docx")
        .unwrap()
        .expect("SDP 应识别为 docx");
...
fn corpus_srs_full_extract() {
    let Some(srs) = corpus("srs.docx") else {
        return;
    };
    let doc = try_extract(&srs, "docx")
        .unwrap()
        .expect("SRS 应识别为 docx");
    assert!(matches!(doc.kind, DocKind::Docx));
    ...
    assert!(
        count_model_tables(&model.body) >= 20,
        "SRS 表格数异常: {}",
        count_model_tables(&model.body)
    );
}
...
fn corpus_sdp_chunks_paginate() {
    ...
    assert!(
        chunks[0].text.starts_with(title.as_str()),
        "首块未含封面标题"
    );
        ...
        .map(|b| match b {
            ...
            Block::Table(t) => t
                ...
                .map(|r| {
                    r.cells
                        .iter()
                        .map(|c| count_model_paragraphs(&c.blocks))
                        .sum::<usize>()
                })
                .sum(),
        })
...
fn corpus_model_text_equals_golden_scanner() {
    let Some((sdp, srs, install)) = all_corpus() else {
        return;
    };
...
fn corpus_inspect_three_projections() {
    ...
    for (name, bytes, has_revisions) in [
        ...
        let err = inspect_document(
            ...
            &InspectRequest {
                projection: InspectProjection::Text,
                start: Some(report.total_blocks + 1),
                end: None,
                row: None,
                cell: None,
                style: None,
...
fn corpus_numbering_values_rendered() {
    let Some((sdp, srs, _install)) = all_corpus() else {
        return;
    };
    ...
    assert!(
        report
            .content
            .lines()
            .any(|l| l.contains("H1 ") && l.contains("列表 1 ")),
    ...
    .unwrap();
    assert!(
        report.content.lines().any(|l| l.contains("列表 ") && {
            let v = l
                .split("列表 ")
                .nth(1)
                .unwrap_or("")
    ...
    for (name, bytes) in [("SDP", &sdp), ("SRS", &srs), ("INSTALL", &install)] {
        ...
        assert!(
            report
                .content
                .lines()
                .any(|l| l.contains("页眉") && l.contains(':') && !l.contains("（空）")),
            "{name} 应有非空页眉行"
        );
        assert!(
            report
                .content
                .lines()
                .any(|l| l.contains("页脚") && l.contains(':') && !l.contains("（空）")),
            "{name} 应有非空页脚行（页码域缓存值）"
        );
...
fn corpus_install_revisions_visible_in_format() {
    let Some(install) = corpus("install.docx") else {
        return;
    };
    ...
    .unwrap();
    assert!(
        text.content.lines().count() > 100,
        "正文行数过少: {}",
        text.content.lines().count()
    );
}
...
fn corpus_edit_engine_roundtrip() {
    ...
    for (name, bytes) in [("SDP", &sdp), ("SRS", &srs), ("INSTALL", &install)] {
        ...
        let t3 =
            pick_editable_block(&xml, &spans, &model, &[t1, t2]).expect("{name} 无第 3 可编辑块");
        ...
        let (new_bytes, applied) = super::apply_edits_to_bytes(
            ...
            &[
                ...
                super::EditOp::DeleteBlock {
                    block: t3,
                    expect_prefix: prefix_of(t3),
                },
            ],
        )
        ...
        .unwrap();
        assert!(
            seg.content.contains(REPLACED),
            "{name} t1={t1} 替换未生效:\n{}",
            seg.content
        );
        assert!(
            seg.content.contains(INSERTED),
            "{name} t2={t2} 后插入未生效:\n{}",
            seg.content
        );
...
fn corpus_set_format_surgery() {
    ...
    for (name, bytes) in [("SDP", &sdp), ("SRS", &srs), ("INSTALL", &install)] {
        ...
        let target = pick_editable_block(&xml, &spans, &model, &[]).expect("{name} 无可编辑块");
        let t2 =
            pick_editable_block(&xml, &spans, &model, &[target]).expect("{name} 无第 2 可编辑块");
        let prefix_of = |n: usize| -> String {
            let mut t = String::new();
            super::docx_model::blocks_text(&model.body[n - 1..n], &mut t);
            t.trim().chars().take(4).collect()
...
fn corpus_ppr_element_surgery() {
    ...
    for (name, bytes) in [("SDP", &sdp), ("SRS", &srs), ("INSTALL", &install)] {
        ...
        let other =
            pick_editable_block(&xml, &spans, &model, &[numbered]).expect("{name} 无第 2 可编辑块");
        let prefix_of = |n: usize| -> String {
            let mut t = String::new();
        ...
        let model2 = super::docx_model::build_document(&xml_dom::parse(&new_xml).unwrap());
        assert_eq!(model2.body.len(), model.body.len(), "{name} 块数应守恒");
        let Block::Paragraph(p) = &model2.body[numbered - 1] else {
            panic!()
        };
        assert!(p.props.numbering.is_none(), "{name} 段级编号引用应消失");
        let new_spans = super::docx_edit::locate_blocks(&new_xml).unwrap();
        assert!(
            new_xml[new_spans[other - 1].start..new_spans[other - 1].end].contains("<w:keepNext"),
            "{name} keepNext 未落位"
        );
        let (mut old_t, mut new_t) = (String::new(), String::new());
...
fn corpus_table_surgery() {
    ...
    fn table_clean(t: &super::docx_model::Table) -> bool {
        ...
        !t.rows.is_empty()
            && t.rows
                .iter()
                .all(|r| r.cells.iter().all(|c| blocks_clean(&c.blocks)))
    }
    ...
    for (name, bytes) in [("SDP", &sdp), ("SRS", &srs), ("INSTALL", &install)] {
        ...
        let Some((tbl_block, row, cell, rows0)) = found else {
            continue;
        };
        ...
        let at = if anchor < tbl_block {
            tbl_block + 1
        } else {
            tbl_block
        };
        let Block::Table(t2) = &model2.body[at - 1] else {
            panic!("{name} 目标块应仍是表")
        };
        ...
        .unwrap();
        assert!(
            grid.content.contains(&format!("[{at}] ▦ 表")),
            "{name} 网格头行缺失"
        );
...
fn corpus_table_format_surgery() {
    ...
    for (name, bytes) in [("SDP", &sdp), ("SRS", &srs), ("INSTALL", &install)] {
        ...
        'tables: for (i, b) in model.body.iter().enumerate() {
            ...
            for row in &t.rows {
                for w in row.cells.windows(2) {
                    let (a, b2) = (&w[0], &w[1]);
                    if a.grid_span.is_none()
                        && b2.grid_span.is_none()
                        && a.v_merge.is_none()
                        && b2.v_merge.is_none()
                    {
                        found = Some(i + 1);
        ...
        let Block::Table(t0) = &model.body[tbl - 1] else {
            unreachable!()
        };
        ...
        let (h_row, h_cell) = {
            ...
            'pick: for (ri, row) in t0.rows.iter().enumerate() {
                for w in row.cells.windows(2) {
                    if w[0].grid_span.is_none()
                        && w[1].grid_span.is_none()
                        && w[0].v_merge.is_none()
                        && w[1].v_merge.is_none()
                    {
                        pick = (
        ...
        assert!(
            tblpr.content.contains("<w:tblPr"),
            "{name} tblpr 应渲染原文"
        );
        ...
        let model2 = super::docx_model::build_document(
            &xml_dom::parse(&super::docx::read_document_xml(&b2).unwrap()).unwrap(),
        );
        assert_eq!(model2.body.len(), model.body.len(), "{name} 块数应守恒");
        let (mut old_t, mut new_t) = (String::new(), String::new());
        ...
        let Block::Table(t2) = &model2.body[tbl - 1] else {
            panic!()
        };
        ...
        let model_m = super::docx_model::build_document(
            &xml_dom::parse(&super::docx::read_document_xml(&m1).unwrap()).unwrap(),
        );
        let Block::Table(tm) = &model_m.body[tbl - 1] else {
            panic!()
        };
        assert_eq!(
            tm.rows[h_row - 1].cells.len(),
            t0.rows[h_row - 1].cells.len() - 1,
            "{name} 横并应少一格"
        );
        assert_eq!(
            tm.rows[h_row - 1].cells[h_cell - 1].grid_span,
            Some(2),
            "{name} 横并跨度应为 2"
        );
        ...
        let model_s = super::docx_model::build_document(
            &xml_dom::parse(&super::docx::read_document_xml(&s1).unwrap()).unwrap(),
        );
        let Block::Table(ts) = &model_s.body[tbl - 1] else {
            panic!()
        };
        ...
        assert!(
            row_s.cells.iter().all(|c| c.grid_span.is_none()),
            "{name} 横拆后应无跨度"
        );
        ...
        let model_v = super::docx_model::build_document(
            &xml_dom::parse(&super::docx::read_document_xml(&m2).unwrap()).unwrap(),
        );
        let Block::Table(tv) = &model_v.body[tbl - 1] else {
            panic!()
        };
        assert_eq!(
            tv.rows[0].cells[v_head - 1].v_merge.as_deref(),
            Some("restart"),
            "{name} 纵并头应为 restart"
        );
        assert_eq!(
            tv.rows[1].cells[v_head - 1].v_merge.as_deref(),
            Some("continue"),
            "{name} 纵并续格应为 continue"
        );
        ...
        let model_v2 = super::docx_model::build_document(
            &xml_dom::parse(&super::docx::read_document_xml(&s2).unwrap()).unwrap(),
        );
        let Block::Table(tv2) = &model_v2.body[tbl - 1] else {
            panic!()
        };
        assert!(
            tv2.rows
                .iter()
                .all(|r| r.cells.iter().all(|c| c.v_merge.is_none())),
            "{name} 纵拆后应无 vMerge"
        );
...
fn corpus_def_style_surgery() {
    ...
    for (name, bytes) in [("SDP", &sdp), ("SRS", &srs), ("INSTALL", &install)] {
        ...
        let list = inspect_document(
            bytes,
            &InspectRequest {
                projection: InspectProjection::Styles,
                start: None,
                end: None,
                row: None,
                cell: None,
        ...
        let target = all
            ...
            .filter_map(|s| {
                children
                    ...
                    .find(|c| {
                        c.name == "w:style"
                            && child_attr(&styles_xml, c, "w:styleId").as_deref()
                                == Some(s.id.as_str())
                    })
                    .map(|c| (s, c))
            })
        ...
        let Some((style_def, _)) = target else {
            continue;
        };
        ...
        let def = inspect_document(
            bytes,
            &InspectRequest {
                projection: InspectProjection::Styledef,
                start: None,
                end: None,
                row: None,
                cell: None,
        ...
        let (new_bytes, applied) =
            super::apply_style_edits_to_bytes(bytes, std::slice::from_ref(&op))
        ...
        assert!(
            applied[0]
                .target
                .as_deref()
                .is_some_and(|t| t.contains("pPr/shd")),
        ...
        assert_eq!(
            super::docx::read_entry(&new_bytes, "word/styles.xml")
                .unwrap()
                .unwrap(),
            super::docx::read_entry(&new_bytes2, "word/styles.xml")
                .unwrap()
                .unwrap(),
        ...
        // 等非样式直接子级逐字节不变
        let new_xml = super::docx::read_entry(&new_bytes, "word/styles.xml")
            .unwrap()
            .unwrap();
        ...
        assert_eq!(
            children.len(),
            new_children.len(),
            "{name} styles 直接子级数应守恒"
        );
        for (a, b) in children.iter().zip(new_children.iter()) {
            let (sa, sb) = (&styles_xml[a.start..a.end], &new_xml[b.start..b.end]);
            let is_target = a.name == "w:style"
                && child_attr(&styles_xml, a, "w:styleId").as_deref() == Some(sid.as_str());
            if is_target {
        ...
        .unwrap();
        assert!(
            def2.content.contains(FILL),
            "{name} styledef 读回应含新底纹"
        );
        let sheet2 = parse_styles(&xml_dom::parse(&new_xml).unwrap());
        assert_eq!(
            sheet2.all_styles().len(),
            sheet.all_styles().len(),
            "{name} 样式计数应守恒"
        );
        ran += 1;
    }
    assert!(
        ran >= 2,
        "SDP/SRS 语料应均有 outline 样式可跑（实际 {ran} 份）"
    );
}
...
fn corpus_def_numbering_surgery() {
    ...
    let mut ran = 0usize;
    for (name, bytes) in [("SDP", &sdp), ("SRS", &srs), ("INSTALL", &install)] {
        let Some(numbering_xml) = super::docx::read_entry(bytes, "word/numbering.xml").unwrap()
        else {
            continue;
        };
        ...
        let list = inspect_document(
            bytes,
            &InspectRequest {
                projection: InspectProjection::Numbering,
                start: None,
                end: None,
                row: None,
                cell: None,
        ...
        let (new_bytes, applied) =
            super::apply_numbering_edits_to_bytes(bytes, std::slice::from_ref(&op))
        ...
        assert_eq!(
            super::docx::read_entry(&new_bytes, "word/numbering.xml")
                .unwrap()
                .unwrap(),
            super::docx::read_entry(&new_bytes2, "word/numbering.xml")
                .unwrap()
                .unwrap(),
        ...
        let new_numbering = super::docx::read_entry(&new_bytes, "word/numbering.xml")
            .unwrap()
            .unwrap();
        ...
        .unwrap();
        assert!(
            drill2.content.contains("lvlRestart"),
            "{name} 下钻读回应含新元素"
        );
        ran += 1;
    }
...
fn corpus_def_change_guard() {
    let Some((sdp, _srs, _install)) = all_corpus() else {
        return;
    };
    use super::def_edit::{child_attr, root_children};
    let styles_xml = super::docx::read_entry(&sdp, "word/styles.xml")
        .unwrap()
        .unwrap();
    let children = root_children(&styles_xml).unwrap();
    ...
    let hit = children
        ...
        .find(|c| {
            c.name == "w:style"
                && !c.self_closed
                && child_attr(&styles_xml, c, "w:styleId").is_some()
        })
        .expect("SDP 应有常规样式");
...
fn corpus_validate_assertions() {
    ...
    for (name, bytes) in [("SDP", &sdp), ("SRS", &srs), ("INSTALL", &install)] {
        ...
        let mut asserts = vec![super::AssertSpec::BlockCount {
            equals: model.body.len(),
        }];
        if let Some(ti) = model.body.iter().position(|b| matches!(b, Block::Table(_))) {
            let Block::Table(t) = &model.body[ti] else {
                unreachable!()
            };
        ...
        let wrong = [super::AssertSpec::BlockCount {
            equals: model.body.len() + 1,
        }];
        let report = super::validate_document(bytes, &wrong).unwrap();
        assert!(!report.passed);
        assert_eq!(report.failed, 1);
        assert!(
            report.failures[0].detail.contains("实际"),
            "{name} 失败明细应含实际值"
        );
        ran += 1;
    }
