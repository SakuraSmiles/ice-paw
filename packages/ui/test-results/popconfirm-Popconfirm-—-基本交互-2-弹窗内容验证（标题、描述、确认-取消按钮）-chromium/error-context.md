...
- Name: popconfirm.spec.ts >> Popconfirm — 基本交互 >> 2. 弹窗内容验证（标题、描述、确认/取消按钮）
- Location: e2e/popconfirm.spec.ts:78:3
...
Locator: locator('.ip-popconfirm__popover').first().locator('.ip-button--primary')
Expected: visible
Timeout: 5000ms
...
Call log:
  ...
  - waiting for locator('.ip-popconfirm__popover').first().locator('.ip-button--primary')
...
```ts
  ...
  11  |  *  7. danger 样式验证
  12  |  *
  13  |  * 预览站：必须运行于 http://localhost:5173
  14  |  * 夹具：/test/fixtures
  15  |  *
  16  |  * 注意：Popconfirm 弹层使用 <Teleport to="body">，因此 popover 不在
  17  |  * wrapper 的 DOM 子树内。需要用 page.locator('.ip-popconfirm__popover')
  18  |  * 在页面级别查找弹层。
  19  |  */
  20  | 
