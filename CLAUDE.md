...
- **channel_hint 段**：`context/system_prompt.rs` 频道协作纪律常量（仅 kind='channel' 由 runner 填充）——@ 礼则（提名字≠指令、只认精确成员名）、接力纪律、诚实边界；anatomy 已收编为 `system_channel_hint` 段（0.7 批③ 组成体检自动覆盖）。
...
- `cargo test --lib` 1488 passed / 0 failed（+ 集成测试：session_runner_e2e 12、session_reconcile_e2e 6+2 ignored、session_event_log_e2e 3、memory_e2e 3、message_repo 7、provider 11）；clippy --tests -D warnings 0 警告；vitest 547（1488/547 = MA-3 二轮批后 1473/514 + 审计批 8 + **频道批 C +25**：批①② Rust 15（channel.rs 引擎/路由/护栏/选举/换帅）+ 前端 25——ChannelNotice 7 / ChatInput.channel 6 / ChatMessages.channel 5 / Sidebar.channel 6 / useTrajectory 频道行 1 + 词表结构锁更新）
