...
- **sender enrichment 三字段**（`message_cmd.rs`，非表列派生）：`sender_agent_id`（发言成员；sweep 落库）+ `sender_agent_name`（发言时名快照——改名不追溯，migration 零新列）+ `turn_duration_ms`（回合耗时，C8b 区间标注）。回填触发判据 = 页内任一行带 sender_agent_id（`pre_materialized` 守卫——普通会话零开销直过；快照缺失回退「已退出成员」）。
