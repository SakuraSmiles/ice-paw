...
## 当前状态（2026-08-17）
- 版本 **0.3.7 已发布**（= 0.3.6 + S1 三件套 + termLabels 收敛；真机验收五项全绿）
- 分支：仅 `main`
- 近期递进：S 批次结构减法（S2-S7 全清）→ backfill → 0.3.6 发版 → **S1 Phase 2B legacy 读路径退役三件套全清 + 真机验收五项绿收官**（恒 Derive + 摘要锚点 seq 化 + Image 双份存储治理，详见「会话事件日志」段）→ 0.3.7 发版
