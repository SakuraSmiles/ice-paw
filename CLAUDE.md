...
- **接力链与护栏四件**（常量在 channel.rs 模块顶）：单链连续成员回合 ≤8（`MAX_CHAIN_TURNS`，达限 `chain_limit`）；同一有序对（发起→被@）链内 ≤2（`MAX_PAIR_REPEAT`，第三次拦 `pair_repeat`）；单消息 @ ≤5（`MAX_MENTIONS_PER_MSG`，超限截断+诚实事件）；成员唤醒频率 10 分钟窗每成员 ≤6（用户点名不占，`frequency`）。拦截事实六 blocked_reason：pair_repeat/chain_limit/frequency/user_preempted/ambiguous_name/coordinator_failed。
...
- `cargo test --lib` 1494 passed / 0 failed（+ 集成测试：session_runner_e2e 12、session_reconcile_e2e 6+2 ignored、session_event_log_e2e 3、memory_e2e 3、message_repo 7、provider 11）；clippy --tests -D warnings 0 警告；vitest 557（1494/557 = 频道批 C 1488/547 + 实测修复批 +5 前端[ChatMessages ⑤ 回归锁] + ② 简报重构两测试替换净 +1 Rust）
