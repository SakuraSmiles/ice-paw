...
│                     #   message/provider/model_profile/screen/inbox/channel）
├── harness/          # 核心业务逻辑
...
│   ├── channel.rs    # 频道 v1 串行共享流（路由/接力/护栏/自选举/换帅/链状态/事实简报）
...
├── composables/      # 前端逻辑组合（useChatEvents/useModelProfiles/useProjectTrajectory/useChannel 等 17 个）
...
- **词表 18 kind** + typed emitters：`harness/event_log.rs`（EventCtx + warn-only 影子定位；词表以 event_log.rs typed emitter 集为单一真相源——`context_breakdown` ③ 可观测化装整回合的段级组成+指纹+逐轮 usage 序列，emit 单点在 stream_loop wrapper（inner 返回后，恒落 turn_ended 之后），derive skip 臂同 turn_context 构建期元数据；另有 plan_updated / model_switch / cross_session_message 族）
...
- **消费引擎**（`harness/inbox.rs`）：触发源——投递时（accept 或 is_reply×hold）且空闲即 spawn / accept 会话回合结束（turn_ended 广播 → drain watcher 2s 轮询等静默、30s 上限 → 链式排空）/ hold 会话用户收件箱批准。**settled(consumed) 在 chat_state.start 成功后、spawn 前 append**（原子占位防双击双消费）。回合成败都算已处理（失败在会话内有 message_error 事实）。
...
### 频道 v1 串行共享流（0.7 批 C，2026-09-10 落地四批；docs/channel-v1-design.md 为设计真相源）
项目成员的群聊式协作——**有主持的串行圆桌**：同一项目全体成员（project_agents）共用一条消息流，一时刻仅一成员发言（chat_state 单写者天然保证）、@点名必应答、广播由统筹者编排、护栏截断接力风暴、人始终在场且治理权最大。与委派/MA-3 三分工：委派=同步阻塞父子任务单元；MA-3=异步跨会话信箱；频道=同项目成员**共享一条流**的对话单元。
- **数据模型（零新表）**：频道 = `conversations` 表 `kind='channel'` + `project_id` 必填的一行；migration 54 建部分唯一索引 `idx_channel_per_project ON conversations(project_id) WHERE kind='channel'`（一项目一活频道）+ 顺路加 `archived_at` 列（频道归档）。成员 = project_agents 现读（变更即时生效，无快照同步义务）；侧栏「开启频道」懒建（`ensure_channel` 幂等——项目创建**不**预建，老项目零迁移、空项目不产生空会话；无成员 Err 三段式指路）。**灵活性天花板显式锁死**（C1 勿翻案）：无跨项目/多频道/散落频道、全员强制在频道。
- **路由模型**：用户消息两路——① `input.mentions`（结构化 @ 点名，上限 5、与成员表比对、非成员静默剔除）→ 被 @ 成员各跑一回合；② 无 mentions = 广播 → 统筹者接令（应答或 @成员 分派）。成员回合复用 `run_agent_turn` 全链路（预算/工具/hooks/事件/降级链照常）；成员回复中的 @ 由后端 `parse_agent_mentions` 从文本解析（**最长前缀精确匹配 / 重名歧义不猜 / email 防御 / 去重保序**——前端 `ChatInput.collectMentions` 同构，两端解析语义勿单边改）。
- **接力链与护栏四件**（常量在 channel.rs 模块顶）：**接力入队按目标成员去重（`enqueue_relay_hops`，2026-09-11 ⑧）**——同目标已在队不重复入队、该跳 from 刷新为最近一次 @ 者（共享流保证被唤醒成员一次读全流中全部指向它的 @，无需逐条各醒一次；治生产风暴：统筹者 multi-@ 与成员报告尾再 @ 叠加出重复入队 → 陈旧提及晚到出队唤醒已完成工作者白跑 + 队列膨胀顶 chain_limit；通知条「X 点名 Y」随 from 刷新与紧邻历史对得上）；单链连续成员回合 ≤8（`MAX_CHAIN_TURNS`，达限 `chain_limit`）；同一有序对（发起→被@）链内 ≤2（`MAX_PAIR_REPEAT`，第三次拦 `pair_repeat`）；单消息 @ ≤5（`MAX_MENTIONS_PER_MSG`，超限截断+诚实事件）；成员唤醒频率 10 分钟窗每成员 ≤6（用户点名不占，`frequency`）。拦截事实六 blocked_reason：pair_repeat/chain_limit/frequency/user_preempted/ambiguous_name/coordinator_failed。
- **C8 在途插话（不拦截，群聊真实体验）**：生成中用户发送 = **物化无条件成功**（新链头）+ 在途接力链取消（`user_preempted` 事件）；连发聚合——新链头启动前 3s 静默窗口（`CHAIN_HEAD_QUIET`，窗口内新用户消息重置计时「说完一起听」防碎链）；前端不做乐观 push（靠 user_message 事件驱动 loadMessages 权威落流）+ queued notice 5s 提示。**「生成中发出」时间区间还原**（C8b）：消息行带 `turn_duration_ms`，前端标注用户消息是否落在上一回合生成窗口内（无 duration 诚实不标注）。
- **统筹者两档治理（C5）**：**系统档**（自选举产生）故障保守换帅——接令回合连续失败 2 次（`COORD_FAIL_STREAK`；失败口径 = turn_ended.termination='error'，interrupted/length 不归咎）罢免 + `failed-over` + 投影回落 joined_at 最早成员 + 清链；**指定档**（用户 `set_channel_coordinator` 任命，`coordinator_is_user_appointed` 出身判定——appointed=true / elected·failed-over=false）故障**不**自动换帅（用户钦点不擅动），频道 popover 给「让系统补选」出口（`reelect_channel_coordinator`——**绕过全员弃权熔断**，用户治理权最大；election_running 守卫照常防双跑）。
- **自选举（一次性，非全响应）**：统筹空缺时**首次广播**触发（打开频道/ensure 不触发——避免无人发起的投票发言）——成员互投 mini 回合（`ELECTION_VOTE_MAX_TOKENS=128`，投票本身就是真实请求 = 兼任健康检查，连不上的成员天然弃权）；平票裁决 = joined_at 最早；全员弃权 = 胜者空缺（广播降级态诚实披露 + 熔断防每条消息重选）。v1 简化披露：投票 prompt 只给成员名清单（不带履历）。
- **系统事实分层（C10b，append-only 原则）**：**内容性事实物化**（成员回复 = messages 行，带 sender 权威）；**行为性事实只进事件**（点名/选举/统筹位三 kind：`channel_mention`/`channel_election`/`channel_coordinator`——词表 18→21，typed emitter 在 event_log.rs，turn_id/message_id 锚链头 `chain:{链头用户消息id}` 三侧同归组）；对 agent 的告知走**派生简报**（`compose_channel_brief`——回合注入事实块 **+ 成员名册（统筹者标（统筹））**，**不产生 messages 行**；三语境：广播接令[统筹职责——适合谁答就 @谁分派、可多 @、不必事事亲自答] / 用户 @ 点名 / 成员接力[from 名]，均带名册——**名册是 @ 接力的可用性地基**，2026-09-11 ⑥ 生产实案：prompt 无名册无角色语境 → 统筹者全亲答、被推着问人时转用 delegate[子会话频道不可见，四成员隐形答完频道只见汇总，群聊感消失]）。derive skip 臂含三 kind（对账零污染）。**channel_mention 的 from_agent_id=None 一值两义由 `broadcast` 位分野（2026-09-11 实测修复）**：true = 无 @ 广播由统筹者接令、false/缺席 = 真 @ 点名（旧事件零迁移）；聊天区**不回显用户自起的首跳派发**（from=null 且未拦截——答案气泡已带身份头，回显即噪音；生产实案：每条广播都出「用户 点名 X 接力」被用户误读成自己 @ 过），护栏拦截与成员接力保留，完整事实流在轨迹页。
- **sender enrichment 三字段**（`message_cmd.rs`，非表列派生）：`sender_agent_id`（发言成员；sweep 落库）+ `sender_agent_name`（发言时名快照——改名不追溯，migration 零新列）+ `turn_duration_ms`（回合耗时，C8b 区间标注）。回填触发判据 = 页内任一行带 sender_agent_id（`pre_materialized` 守卫——普通会话零开销直过；快照缺失回退「已退出成员」）。**出生打标三出生点（0.7 批 C 实测修复①③b，2026-09-11）**：session_runner 首占位（chat:start emit 前）/ loop_engine 阶段 H 下轮占位（chat:assistant-start emit 前）/ channel.rs cast_vote 投票行——`(conv.kind == "channel").then(agent.id)` 派生，经 StreamLoopInput→LoopConfig.sender_agent_id 穿 spawn 边，`repo::message::set_sender_agent` 落列（warn-only）；出生即带成员归属，sweep 的 IS NULL 兜底天然跳过（互锁测试 birth_stamped_sender_survives_sweep）；仅频道打标，1v1 None 零开销。
- **删除守卫两件**：删 agent → 频道行 agent_id 迁投影（历史消息不丢，最后成员拒删 agent）；项目永久删除 → 频道**归档保留**（archived_at 置位；记录只读：输入 disabled + placeholder「频道已归档」+ send 拒三段式；散落 scope 侧栏「已归档」行；归档频道 reelect 拒）。
- **channel_hint 段**：`context/system_prompt.rs` 频道协作纪律常量（仅 kind='channel' 由 runner 填充）——@ 礼则（@名字+任务简述、只认精确成员名）、接力纪律、**共享记忆句（频道历史是全体成员共享记忆——他人报告过的路径/发现/结论可直接取用）**、不重复劳动、诚实边界（2026-09-11 ⑦ 换装——原 delegate 可见性披露句随频道禁用 delegate 一并退役，见不变式③）；anatomy 已收编为 `system_channel_hint` 段（0.7 批③ 组成体检自动覆盖）。
- **命令四件**（`commands/channel_cmd.rs`）：ensure_channel（懒建；**title = 项目名本体**——2026-09-11 拍板频道身份改由头部 tag 徽章呈现，不再拼「 · 频道」后缀，migration 55 剥存量行）/ get_channel（视图：频道行+成员档案+统筹位+appointed 出身）/ set_channel_coordinator（任命 Some/罢免 None——同目标幂等）/ reelect_channel_coordinator（补选）。**前端**：Sidebar 频道区块（活频道一行「统筹 · 名」/「待选举」诚实边界 + 懒建入口 + 失败行内 caption + streaming 指示；**搜索框已整体移除**，用户拍板 2026-09-10）/ **ChatHeader 频道形态（2026-09-11 改版⑪）**——tag 徽章紧跟标题（无 # 图标；归档态 warning 色「已归档」）+ 子标题成员头像叠层（前 4 枚 sm 20px 负 margin 叠压 + 溢出「+N」+「N 名成员」+ Shield 统筹名/待选举灰字；不显统筹者模型——广播路由下每人可发言，显某人的模型是误导）+ 右侧统筹者胶囊换 Users 图标方钮开 ChannelPopover（名字收进子标题不重复占位）+ 频道态不渲染 xl 大头像（统筹者投影换帅跳变）/ ChannelPopover（成员列表+统筹位治理）/ ChatMessages sender 分组（不同成员绝不合并；**群聊气泡布局（2026-09-11 拍板⑩）**——`.assistant-wrap` 频道态 grid 两列：头像独立气泡外左列（EntityAvatar lg 36px）+ 身体列（昵称行 body-sm 醒目化在气泡上方 + `.assistant-body` 气泡体承接原组容器的背景/圆角/padding，外层退位透明；1v1 态 wrap/body 双层透明直通零视觉变化；匿名组仅 body 一个孩子须显式 grid-column:2 定位防塌进头像列）；**live 透出**——chat:start 频道恒权威刷新 loadMessages + assistant-start 占位继承上一条 assistant 的 sender，流式期间头像昵称同样在场）/ 频道交错单元（Interstitial：通知条 ChannelNotice + **选举聚合卡 ChannelElectionCard**——按 created_at 与消息组交错，组前 preInterstitials+尾部尾巴；**每个交错单元只消费一次**——挂到首个开始时间不早于它的组（双指针线性扫，比较符含等号：DB 秒级时间戳下通知事件[派发时刻落库]与目标成员占位行[Pipeline 后几毫秒]常落同一墙钟秒，严格小于会把通知滑过该成员自己的组、呈现在其答完之后——2026-09-11 实测修复⑨；⚠️勿退回每组 filter 全量 time<start：历史卡随每条新消息重复渲染 + TransitionGroup 重复 key 错位，2026-09-11 生产实案⑤）；**未拦截接力通知吸入被点名成员气泡身份行（2026-09-11 拍板⑪，⑫ 图标化）**——居中条退位为昵称行尾一枚 12px 图标标注（**两态分野发起者**：AtSign=用户直接 @ 点名 / CornerUpRight=成员间接力[统筹分派或成员转 @]，发起者与完整句义进 hover title「{from} 点名 {to} 接力」/「用户 点名 {to}」——文字形态「@来源名」易被误读成气泡主人 @ 了谁，⑫ 弃文字），判据 = 未拦截 ∧ 发起可归因（from≠null，或 from=null 真 @ 点名非 broadcast——broadcast=true 广播接令不吸，统筹者直接应答是默认对话流无需标注）∧ to==组 sender ∧ assistant 组（absorbableInto 类型谓词）；**吸入候选挂不上当前组时延递（2026-09-11 ⑬）**——首跳点名事件与 user 消息行常落同一墙钟秒（路由在物化后即刻派发）、`<=` 平局挂组下先被 user 组吃掉，multi-@ 其余跳撞同秒他人组同族；候选进 carry 向后续组递送直至目标组或尾部（isDeferrableMention 与 absorbableInto 共用候选判据；护栏拦截/广播/选举卡不延递——事实性内容按时间就地呈现；延递项不进任何消费集，落空兜底从「下一组前居中」移尾部 = 目标不在加载窗口/已退出的待接力尾象）；护栏拦截/换帅类保留居中条；⚠️ 吸入 key 必须进 tail 消费集（RenderGroup.absorbedKeys——漏收 = 尾部双渲染）；一届选举聚一张卡，投票行气泡跳过、配对 elected 通知抑制；**广播接令不出通知条**（useChannel 过滤只拦 `broadcast === true`——真 @ 点名放行进吸入路径成 AtSign 图标；broadcast 缺席位的④前旧事件按点名渲染，serde 契约既定容忍）；useChannel 模块级单例三视图）/ ChatInput @ 弹层（候选=成员、统筹者 owner 标注、文本保留 `@名字 ` 无 chip——**文本即单一真相源**）/ 轨迹三 kind 归 CROSS 行（短码摘要，纯函数无 store）。
- **⚠️ 不变式**：① 单写者仲裁由 chat_state.start 自然保持——接力触发一律走「等静默轮询」（QUIET_POLL 2s / QUIET_TIMEOUT 30s，inbox drain 同款；start 失败=忙→重试，结构性无并发）；② 频道回合与 1v1 同链路（MemoryStage 摘要照接——摘要按会话维度本就共享）；③ 工具注册按会话 kind 分野——**delegate / relay / read_reference 均仅 1v1**（kind='chat'；2026-09-11 ⑦ 翻转原「delegate 频道同权」拍板——三轮实案：委派子会话读不到频道历史、回传「看不到代码」固化统筹者错误信念、@ 被绕空，委派在频道=信息断流通道；**delegation_hint 可调度清单注入同闸收窄**——清单指向不存在的工具=「prompt 说能委派但工具不在场」矛盾；read_reference=@ 引用快照钻取是 1v1 语义——频道 @ 纯寻址不产快照[见 C9]、委派子会话无用户 @，调用结构性必败，⑧ 批收口：组装期 `snap.remove`，前两工具走注册闸、路径不同闸相同）；④ 词表三处同步纪律照旧（event_log typed emitter + derive skip 臂 / EV_KIND_TO_FILTER + summarizeEvent / 前端通知条词表）；⑤ mentions 解析两端同构——改 parse_agent_mentions 必同步 collectMentions（及各自测试）；⑥ 归档频道一切写路径拒绝（send/reelect/统筹治理），只读是契约不是建议；⑦ **选举聚合只发生在渲染前**（useChannel.applyEvents——数据层/轨迹/append-only 事件零改），投票行跳过判据 = 窗口内 election: 前缀 turn 的 assistant_message id 集（窗口外旧选举无卡 → Set 不含 → 照常气泡自然回退），channel_coordinator(elected) 抑制判据 = 窗口内有卡宣告同一胜者（appointed/removed/failed-over 与无卡 elected 保留）；⑧ **NeedsCoordinator 臂头锚 = backlog.first()**（election_register_head——该臂未派发任何东西，选举后重消费必须回到积压起点；Members 臂保持 last 语义），**sender 出生打标与 sweep IS NULL 兜底互锁**——出生带标行不被兜底盖写、未打标旧行照常 sweep（两条互锁测试锁定，勿只修一侧）；⑨ **轨迹轮号口径四处同步**（buildRows 头 / TrajectoryView stats / Timeline buildSpans / repo count_turns_before）——口径 = **DISTINCT turn_id（新 turn_id 首见递增，seenTurns）**：频道链上同 turn_id 的多跳成员回合被 chain: 派发事件切段后**复用首段号**（段切换 ≠ 新轮——⑮ 批根治「16 真轮膨胀成 47 头号」）；轮外段（`chain:`/`cross:`/`election:` 前缀 + turn_id=NULL 的 channel_coordinator，统一走**事件级谓词 `specialOfEvent`**——coordinator 须看 kind，单看 turn_key 判不出）不占轮号、不画轮边界、头标签特殊化（「频道接力」/「跨会话来件」/「频道选举」/「统筹位变更」）、头时间取段首事件自身、头 key 带段首 seq 后缀防交错段重复；动任一处必须同步其余三处（⑭ 批立、⑮ 批补全口径与漏网前缀）。

## 当前状态（2026-09-11）
- 版本 **0.7.0 已发版 push**（tag v0.7.0 + GitHub Release 带 NSIS exe；= 0.6.16 + **0.7 三批**：批 A 委派子会话降级链继承 db29be7 + 批 B MA-3 跨会话通讯 cacaa70/b9bea59/083f32a/9e430fa + 反馈批 bfebf6b/41816d9/e70d302 + 二轮 a089e62/f09e102/6e2a650+补批 + 三轮散落入口隐藏 + 批③ 上下文开销可观测化 98e4c8d/9cf769f/cb015c4 + **审计修复批 T** 0dd3e2d/55e7420/d23d36a（Esc 栈谓词/在途拦截可见化/消费回合 live 流入/瀑布图词表/文档对齐）+ 收件箱浮层排版 6236d78 + 手测销项 2f4e19a；批④ diff 侧栏已回退（archive/0-7-batch4-diff-sidebar 可捞回，新方向 no-git 工具自算）；**0.7 批 C 频道 v1 串行共享流已落地（2026-09-10，发版后 commit——随下版发布）**；cargo 1473 / vitest 522）
- **0.7 批 C 频道 v1 串行共享流已落地（2026-09-10，四批 commit：① 引擎 41d2bdc + ② 选举换帅 cf4b3db + ③ 前端 e129d46 + ④ 文档；详见「频道 v1」节）**：migration 54 + channel.rs（路由/接力/护栏/自选举/换帅/链状态/事实简报）+ 命令四件 + 前端全线（侧栏频道区块+成员身份分组+@ 点名通路+事件通知条）。cargo 1488 / vitest 547 / clippy 0。v1 简化披露：选举投票 prompt 只给成员名清单（不带履历）；护栏初值（链 8/有序对 2/频率 600s·6/@5）按实际使用优化（用户拍板）。
- **频道 v1 实测修复批已落地（2026-09-11，用户真机三反馈；含首条静默失败 6482d8f + 侧栏统筹误显 6b7148f）**：③ 选举触发错乱根治——NeedsCoordinator 臂头锚原取 backlog.last()，选举后 from(last) 重消费只含末条、**首条广播永久搁浅**；改 `election_register_head`= backlog.first()（该臂未派发任何东西，头锚必须回到积压起点）+ 回归锁测试。③b/① 频道 **sender 出生打标**三出生点（首占位/下轮占位/投票行，经 LoopConfig 穿线 + `set_sender_agent`；修 sweep IS NULL 兜底把选举投票行 sender 全盖成统筹者的误归属 + live 路径头像昵称全程在场——chat:start 频道恒权威刷新 + assistant-start 占位继承上一条 sender）。② **选举聚合卡**（生产反馈「明明是选举一件事，零散信息重复出现」）：useChannel 三视图（electionCards 按 turn_id 一届一卡 / electionVoteIds 投票行跳过 / notices 配对抑制）+ ChatMessages Interstitial 泛化 + ChannelElectionCard 组件——**聚合只发生在渲染前，数据层/轨迹/append-only 零改**（与用户「事件日志契合」解读一致）。④ **广播误标点名**（续聊反馈：无 @ 普通发送每条都出「用户 点名 X 接力」）——channel_mention 增 `broadcast` 位（from=None 两义分野）+ 聊天区不回显用户自起首跳派发 + 轨迹分词「广播 · X 接令」。⑤ **历史选举卡随新消息反复出现**（续聊二轮反馈：每次发言前后都重复出选举卡）——ChatMessages renderGroups 原给每组挂「早于本组开始的全量交错单元」，历史卡被复制到后续每个消息组头上（TransitionGroup 重复 key 再叠错位渲染）；改双指针消费一次（每个单元只挂到首个晚于它的组）+ 回归锁测试。生产库既有 4 条投票行 sender 错值不回修（单测试频道可接受）。cargo 1493 / vitest 557。
- **频道 v1 二轮实测修复⑥已落地（2026-09-11，用户反馈「更像是和统筹者在聊天」群聊感缺失；生产库只读取证 = 频道 19660d0f，行 3519-3557：19 条用户广播全部测试小助手独答 + 「那问问其他人呢」触发 4 个 delegate 子会话[用户不可见]、频道只见统筹者汇总表）**：根因三件——统筹者回合的事实简报无角色语境（不知道自己是统筹者、无分派职责）+ prompt 无成员名册（模型无法可靠 @ 任何人，只能从选举历史猜名字）+ CHANNEL_HINT 未披露 delegate 子会话对频道不可见（被推着「问其他人」时选了熟悉的 delegate）。修法：`BriefSpec::UserBacklog{count}` 重构为 `UserInitiated{broadcast,count}`（区分广播接令 vs 点名）+ `compose_channel_brief` 增 roster 参（三语境均注入名册、统筹者标（统筹）；广播臂带统筹分派职责、点名/接力臂带「可再 @ 他人」）+ CHANNEL_HINT 补 delegate 可见性句（披露而非禁用——当批口径；次日⑦批翻转，见下条）。⚠️ 勿回退：名册注入是 @ 接力的可用性地基，简报措辞改动须同步 channel.rs 两测试。cargo 1494 / vitest 557。
- **频道 v1 三轮实测修复⑦已落地（2026-09-11，用户反馈「两次对话为什么都还是委派？这还是群聊吗」+ 追问「不配 workspace 时其他 agent 应该也能知道路径」）**：生产库取证（频道 19660d0f 第二轮）——统筹者 02:01 三路委派探测，子会话（隔离工作区+读不到频道历史）回传「成员看不到代码」的隔离答案 → 02:25 委派 args 原话「目前我们四位成员都说看不到项目代码」→ **错误信念固化，@ 接力整体被绕空**。归因：不是模型不听话，是 delegate = 信息断流通道（问了但拿不到共享流知识，答案也回不到流里）。修法两件（均通用零场景特化）：① **频道不注册 delegate**——session_runner 两 kind 闸同步收成 'chat'（工具注册 + delegation_hint 清单注入，防「prompt 说能委派但工具不在场」矛盾）；② **CHANNEL_HINT 换共享记忆句**（「频道历史是全体成员的共享记忆——其他成员在流中报告过的路径、发现与结论可直接取用」+ @ 分派带任务简述；原 delegate 可见性披露句退役）。核实件：os_context 平台层已通用披露 cwd +「工作目录外文件直接用绝对路径」（简报注入工作区事实=重复建设，撤）；授权记忆目录级（authority.rs mark_dir_authorized）——首个成员获批一次工作区外路径、同目录全会话免弹。设计稿 C7 整段重写（诚实记录翻转+回滚路径三处同闸）。
- **频道 v1 四轮实测修复⑧已落地（2026-09-11，用户反馈「还是存在比较大的问题」+ 接力提示文字渲染问题；生产库取证 = 频道 19660d0f 第三轮 03:06-03:19：一条广播触发 8 跳接力链跑 10.5 分钟 + 末尾 3 条 chain_limit 拦截）**：⑦ 版本工作面先确认——无 delegate、成员真做工具工作（directory_tree/read_file/git）、共享流知识传播生效（dev-2 直接引用 cb 报告的路径与结论）、sender sweep 归属正确。「大问题」根因三件修复：① **接力队列无去重**（风暴形态 = 统筹者 multi-@ [cb,dev,dev2] 后 cb 报告尾再 @ 同两人叠加重复入队 → dev/dev2 各跑两遍、第二遍已是 5 分钟后陈旧提及醒来「已交付完毕」白跑；每份报告尾 @测试小助手又把统筹者唤醒 3-4 次困惑 hunting @ 原文）——`enqueue_relay_hops`：同目标已在队不重复入队 + from 刷新为最近 @ 者；② **read_reference 频道陷阱**（@ 引用快照钻取是 1v1 语义，频道 @ 纯寻址不产快照——m3 困惑时两次调它 hunting「@ 我的那段」结构性必败）——组装期 `snap.remove` 与 delegate 同闸（注册路径不同：它在 register_builtin 全局注册，靠组装期剔除非注册闸）；③ **频道回合事件 actor 归因**（StreamLoopInput.agent_id 原取 conv.agent_id[统筹者投影] → 改执行成员 agent.id；1v1 两者恒等零变化，与 pre-loop EventCtx 对齐——修 assistant_message/tool_execution 事件 actor 恒为统筹者的轨迹失真）。「渲染问题」定性 = **数据非 CSS**：ChannelNotice 文案构造与 nameOf 解析均正确，观感问题源自己入队快照（出队呈现时与相邻以 @Y 结尾的报告对不上）+ 重复/晚到出队的噪音条成串——①的 from 刷新 + 去重同治。cargo 1495 / clippy 0。
- **频道 v1 五轮实测修复⑨已落地（2026-09-11，用户按测试剧本跑新一轮后发截图问「这个顺序正确吗」；生产库取证 = 频道 19660d0f 第四轮 seq 252-267 + 消息行 r3652-3655）**：**核心行为全绿**（⑧ 生效实证——统筹者一条广播分派 code-buddy/dev-2 两路、每成员恰醒一次[代码助手报告尾也 @ 了 dev-2 但去重闸拦住]、通知 2 from 刷新为「代码助手」与紧邻报告尾句吻合、零 blocked、广播首跳正确过滤、尾部安静）。唯一残留 = **通知条位置呈现层瑕疵**：截图上两条接力通知都出现在被点名成员答完之后。根因（DB 逐秒比对坐实）：channel_mention 事件在派发时刻落库（channel.rs start 成功后 spawn 前）、目标成员 assistant 占位行在 Pipeline 后几毫秒创建——SQLite 时间戳秒级粒度下两者常落**同一墙钟秒**（r3654↔hop2 同 04:38:50、r3655↔hop3 同 04:38:59），而 ChatMessages renderGroups 挂载判据是严格 `<`——平局时通知滑过该成员自己的组、落到下一组前（末组则落尾部）。修法 = 比较符放宽 `<=`（平局挂该组、渲染在回复前）+ 回归锁测试。vitest 558。
- **频道群聊气泡 IM 化已落地（2026-09-11，用户反馈「头像和 agent 名字不够明显，参考常见聊天软件的群聊气泡方式（主要头像昵称，内容与 1v1 一致）」）**：ChatMessages assistant 分支包 `.assistant-wrap`（:class="{channel}"）——频道态 grid 两列（36px 头像列 + 身体列）：头像 EntityAvatar **lg 36px 独立气泡外左列**（grid-row 1/span 2）+ 昵称行（caption-12→**body-sm-13** 醒目化 + Shield）移出气泡上方 + `.assistant-body` 承接原组容器背景/圆角/padding（外层 `.channel-msg` 退位透明）；**1v1 态 wrap/body 双层透明直通零视觉变化**；匿名组（旧消息未 enrich）只有 body 一个孩子——**grid-column:2 显式定位防塌进头像列**（⚠️自动放置陷阱）；用户消息（右侧）不动——诉求聚焦成员消息，IcePaw 无用户身份概念不造假头像。既有断言选择器（.channel-sender-head/.channel-sender-name/.channel-sender-shield）在新结构全兼容 + 新增 ⑩ 布局锁测试。vitest 559。
- **频道头部与接力通知形态改版已落地（2026-09-11，用户两设计任务、AskUserQuestion 三问全拍板推荐项）**：⑪-a **频道标题头部**——标题回归项目名本体（ensure_channel 不再拼「 · 频道」+ migration 55 存量 UPDATE 剥恰 5 字符后缀；SQLite length()/substr() 按字符计、LIKE BINARY 逐字节匹配）+ 紧跟标题 tag 徽章（无 # 图标；归档态 warning 语义色「已归档」）+ 子标题重设计为成员头像叠层（前 4 枚 EntityAvatar sm 20px 负 margin 叠压 + 头部底色描边 + 溢出「+N」+「N 名成员」+ Shield 统筹名/待选举灰字；**不显统筹者模型**——广播路由下每人可发言，显某人的模型是误导；ChannelMemberInfo 无头像字段，经 agent store 按 id 解析）+ 右侧统筹者胶囊换 Users 图标方钮（名字收进子标题不重复占位，点开同一 ChannelPopover）+ 频道态不渲染 xl 44px 大头像（统筹者投影换帅跳变）+ 归档态子标题「已归档 · 记录只读」warning 色（成员表已随项目消亡不渲染叠层）。⑪-b **接力通知吸入气泡身份行**——未拦截成员间接力通知不再居中条，吸入被点名成员气泡昵称行尾成「@来源名」小标注（AtSign 12px + 来源名 tertiary 灰 micro，完整句义进 hover title）；护栏拦截/换帅类与落空兜底（user 组/匿名组/to 不命中）保留居中条；数据层/useChannel/ChannelNotice 组件零改（聚合只发生在渲染前，不变式⑦ 延续）。⚠️ 吸入 key 必须进 tail 消费集（RenderGroup.absorbedKeys 双路收集——漏收 = 已吸入通知在尾部再出一条居中条）。三测试断言随吸入形态更新 + 新增吸入锚定四臂用例（拦截不吸/to 不命中不吸/落 user·匿名组不吸）。cargo 1495 / clippy 0 / vitest 560。
- **频道吸入标注图标化已落地（2026-09-11，用户反馈「@来源名文字易误解为气泡主人 @ 了谁」；⑫ 批）**：吸入标注弃文字改**两态图标**——昵称行尾一枚 12px tertiary 灰图标（显式 inline-block 防 base.css svg 块级化）：**AtSign = 用户直接 @ 点名**（hover「用户 点名 {to}」）/ **CornerUpRight = 成员间接力**（统筹分派或成员转 @，hover「{from} 点名 {to} 接力」）/ **无图标 = 广播直接应答**（频道默认对话流）——发起者识别从文字转图标直观分野，不显示发起者名字、需要时 hover。配套两件：① useChannel 过滤收窄 `!from → !from && broadcast === true`（只拦广播接令；**真 @ 点名放行**进吸入路径——此前 from=null 全拦，点名无图标可显）；② ChannelNotice 兜底分词拆 from=null 分支（首跳点名「用户 点名 X」不带「接力」——接力 = 成员间二跳起）。**broadcast 序列化边界**：后端 serde `skip_serializing_if=Not::not` 使 false 永不落库（JSON 缺席）——from=null+缺席 = 新真点名或④前旧广播（真值不可区分）；判定：不改后端 serde（④ 修复在第一修复批，其后生产事件全带显式位；RENDER_CAP=30 窗口兜底旧广播实际出不了窗，自愈）。三测试文件同步（ChatMessages.channel 图标断言 + 点名/广播分野新用例 / useChannel.election filter 断言 3→4 / ChannelNotice 首跳分词）。vitest 561。
- **频道吸入延递已落地（2026-09-11，用户实测⑫ 批后反馈「直接 @ 点名的信息没有融入气泡」；⑬ 批）**：后端无罪（channel.rs 用户点名路径 Hop 带 from:None + broadcast 缺席，run_next_hop 照常 emit_mention）；根因 = **前端消费时序**——首跳点名事件在消息物化后即刻派发落库，与 user 消息行落**同一墙钟秒**（DB 秒级粒度，与⑨ 取证的「事件↔占位行同秒」同族），renderGroups 双指针按组序推进、user 消息组排前，`<=` 平局判据（⑨ 引入）把点名事件先吃进 user 组（role=user 吸不了）→ 就地 preInterstitial 居中条，永远到不了目标成员的 assistant 组；multi-@ 同秒多跳同族（to=B 候选先撞上同秒的 A 组就地消化）。修法 = renderGroups **延递机制**（carry 缓冲 + 复试循环）：吸入候选挂不上当前组时不就地居中，向后续组递送直至目标组或尾部；护栏拦截/广播/选举卡不延递（事实性内容按时间就地）；落空兜底从「下一组前」移尾部（目标不在加载窗口/已退出的待接力尾象，语义合理）。安全性：carry 项恒 time ≤ 当前组 start（组按时间升序），无时间倒流。两用例锁定：同秒平局越过 user 组 + multi-@ 各归各位。vitest 563。
- **轨迹轮数统计维度统一已落地（2026-09-11，用户问「轨迹里面有 40+ 轮，为什么实际只有十几轮？这个统计维度是否没有统一」；⑮ 批，生产库取证 = 频道 19660d0f：333 事件 16 真轮，表格膨胀出 47 个头号、工具栏 stats 16 正确、后端偏移 17 多 1）**：诊断 = ⑭ 批修复不彻底两处——① buildRows/Timeline 轮号按「段切换」递增（currentTurnKey 判据），而频道链上全体成员回合共用链头 turn_id（频道设计 turn_id==user_msg_id）、回合间穿插 chain: 派发事件 → 同一 turn_id 被切成多段连续段（链头 de72738c 实测 7 段），「真→特殊→同一真」路径每次重入都 +1（⑭ 论证「非特殊 turn_id 会话内不交错」在频道不成立——同 turn_id 交错恰是频道常态）；② `election:{发起id}` 段与 turn_id=NULL 的 channel_coordinator 事件不在⑭排除清单（前者仍占号、后者落孤儿桶错标「纪元前事件」）。修法：轮号改「**新 turn_id 首见递增**」（seenTurns Map——同轮二段复用首段号、头字段取段首事件自身[seq/createdAt]、turnMs=null 一轮一个总耗时只在首段头）；specialTurnOf 补 election: + 新**事件级谓词 specialOfEvent**（`turn_id==null && kind==='channel_coordinator'` → coordinator——emitter 落 NULL，单看 turn_key 判不出），buildRows/stats/buildSpans 三处统一走它；TrajectoryTable turnLabel 词表扩「频道选举」「统筹位变更」；后端 count_turns_before SQL 补 NOT LIKE 'election:%' + NOT(NULL×channel_coordinator)（fixture 扩两行断言守此关）。孤儿桶多段复用同号与后端 DISTINCT COALESCE 口径对齐（接受）。cargo 1496 / vitest 567 / clippy 0。
- **轨迹页轮次划分审计修复已落地（2026-09-11，用户委托「检查轨迹里轮次划分以及收起展开功能是否正常」；⑭ 批）**：审计结论 = 折叠/展开本体正常（toggleTurn 按 turnKey 交错段一致折叠 = 同链同收起是期望行为、collapseAll 唯一键集、搜索强制展开、matchCount 逐段、streaming 行独立尾接、头统计不受 hiddenKinds 影响是设计意图）；真问题三个，全源于**特殊段被当普通轮次**——频道行为事件（turn_id=`chain:{链头id}`）与跨会话来件（`cross:{message_id}`）与成员/消费回合在时间线上**交错**（同 turn_id 非连续多段，非特殊 turn_id 会话内不交错——回合串行、委派独立 session）：① 多段共享头渲染 key `th-${tk}` 重复（v-for 警告/复用错位）；② 特殊段占轮号（后续真轮虚高）+ 无 turn_ended 渲染假「进行中」+ 恒零「0 条回复 · 0 次工具」噪音；③ 后端 count_turns_before 含特殊段，翻页轮号漂移。修法：头 key 带段首事件 seq 后缀（`th-${tk}@${seq}`）；`specialTurnOf` 谓词（剥 `${session_id}::` scope 前缀再判）三处同源——buildRows 头（不占号 + 标签「频道接力」/「跨会话来件」 + 无终止徽/回复统计 + 头时间取段首事件自身[全局聚合首会把段 1 时刻标在段 2 头上] + turnMs=null）/ TrajectoryView stats / Timeline buildSpans（不画轮边界，事件仍上图）；后端 count_turns_before SQL 排除两前缀。**⚠️ 不变式：轮号口径四处同步**（buildRows / View stats / Timeline buildSpans / repo count_turns_before）——动任一处必须同步其余三处 + useTrajectory 交错桶回归锁（同键多段齐折叠语义保持）。cargo 1496 / vitest 565 / clippy 0。
- **MA-3 + 事件日志链路审计修复批已落地（2026-09-10，批次 T 入台账 docs/tech-debt-ledger.md）**：四路扫描 + 生产库只读取证（52 检查项 = 48 成立 + 2 存疑闭合 + 2 文档漂移；生产数据全绿）。行为修复三件 + 词表补角 + 文档对齐五处：① **T1 Esc 栈活跃谓词**——useEscapeStack 模块级栈原只触发栈顶 + stopImmediatePropagation，「常驻 setup 注册 + 条件浮层」形态（ChatHeader 收件箱/删除确认条、Sidebar flyout）的空转回调吞掉本该关浮层的 Esc；修 = Entry 增 `active?: () => boolean` 谓词（Esc 自栈顶向下找首个活跃条目；全不活跃**不消费事件**放行组件自身，如标题编辑取消）；不带谓词 = 恒活跃向后兼容，v-if 挂载形态四调用方零改动。② **T2 sendMessage 早退可见化**——sending 全局单份（消费回合 assistant-start 也置位，承重墙勿动）下静默早退吞输入；修 = 写 send_failed 横幅 + lastFailedSend 重试出口（附件 chips 不消费）。③ **T3 消费回合 live 流入**——外部回合判据 `!ucb && sendingConvId !== cid`（后端 emit_user_blocks=has_files：纯文本用户回合 ucb 也是 None，判据只能靠 sendingConvId）→ sending 即刻置位 + loadMessages 权威刷新 + **跳过本地占位 push**（DB 行已建防重复）；顺手暴露 store 的 sendingConvId/loadMessages（原内部成员，事件层不可达 = TypeError）。④ 瀑布图 laneOf/KIND_LABELS 收编 cross 两 kind（归 User 道外来输入侧 + 镜像 useTrajectory 词表）。⑤ 文档对齐：词表 14→18（derive.rs 3+15 实证）/ drain 2s 轮询·30s 上限 / default_inbox_policy hold→accept 对齐 migration 52 / reconcile 模块头补 incomplete_turn_legacy_rows / event_log 词表真相源指向本文件（migration 44 头注释仅初始集）。B 级防御性缺口七条 + D 级六条全观察池（台账批次 T；最值得留意 = T6 回投失败源会话零告知）。**2026-09-10 用户真机手测通过**（Esc 三连/在途拦截横幅/观看来件 live 流入均符合预期）。cargo 1473 / vitest 522
- **MA-3 二轮实测修复批已落地（2026-09-10，已 commit a089e62/f09e102/6e2a650 + 散落入口隐藏补批）**：用户真机 A↔B 实测两反馈（数据取证见 session_events：B 每轮回 2 条）。① **双投递根治两道闸**——标注按 expect_reply 分叉（true → 指引「直接作答（无需调用投递工具）」，false 保持投递工具指引；两分支保 `target=` 锚）+ `MANUAL_REPLY_GUARD` 防重兜底（消费回合内已手动投回源会话则跳过自动回投，不依赖模型听话；回投 spawn 内 done_rx.await 后检查，登记/检查均内存态）；② **收件箱队列视图**——accept/is_reply 条目「排队中 · 会话空闲后自动处理」标注 + 批准钮降级次要「立即处理」（不摘——配额尽时手动放行是唯一通道），hold 保持真审批「批准并消费」；③ **busy 文案语境分叉**（inbox_cmd auto 语境「已在队列中会自动处理」vs hold「等本轮结束后再批准」）；④ relay.rs 工具 description 补「expect_reply=true 时目标会被指引直接作答，勿自调投递工具回复」。顺手：头部计数 accept 下「N 条排队中」。⑤ **散落会话隐藏收件箱入口**（三轮，用户拍板）——`kind !== 'delegation' && (project_id || inboxPending > 0)`（ChatHeader）。回归核验（用户真机 03:52 时段）全绿：双投递根治实证（两 B 各只回 1 条、send_message_to_session 调用数=0）、回投 1 秒内自动消费全程零手动。cargo 1473 / vitest 514
- **MA-3 测试反馈修复批已落地（2026-09-10，已 commit bfebf6b/41816d9/e70d302）**：用户实测四反馈 + 补充一件。① 默认政策 hold→**accept**（migration 52 DEFAULT 重写——dev 库实测 52 从未成功应用 max=51 改值安全；「在 A 发起、切到 B 批准」反人类）+ **项目边界硬校验**（`project_boundary_error` 纯函数：同项目才放行，跨项目/散落全拒三段式指路；执行期 deliver 内拦非注册期隐藏）；② **expect_reply 回投免 hold 扣**（deliver 增 is_reply 参，accept ∨ (is_reply×hold) 自动消费，refuse 恒拦；回投带 is_reply 元数据全链路至 InboxItem「回复」pill）；③ **popover 层级修复**（ChatHeader z badge→dropdown(100)——popover 困在 header 层叠上下文，badge(10) 输给 tabbar(raised 20)，父层提高子层跟父）；④ **incoming 双块结构化**（[Text 标注, Text 正文] + **incoming_source 元数据三写**：messages 列 migration 53 / user_message 事件 payload serde default / derive 透传；前端 `incomingInfoOf` 元数据优先→parseIncomingText 文本兜底）；⑤ **list_conversations 发现工具**（全量会话概述零内容：id/标题/agent/项目/更新时间，kind='chat' 过滤+倒序+CAP 100 截断；治「投递点对点但 agent 查不到地址、@会话 兼职寻址」语义混乱——可见性 ≠ 写权限，边界拦在投递层）+ toolLabels 两词目。顺手件：inbox_cmd InboxItem.is_reply、useInbox 通知政策分流（accept=知会/hold=待批准）、伪造 49 库测试补 messages/conversations 最小表。cargo 1471 / vitest 506
- **0.7 批③ 上下文开销可观测化已落地（2026-09-10，98e4c8d 后端 + 9cf769f 前端 + 文档 commit；已随 0.7.0 发版）**：BudgetPill miss 归因 chip + 轨迹页「上下文组成」体检区。后端五件套：SystemPromptParts 五段化（joined() 字节等价测试锁）/ `context/anatomy.rs` 事后纯函数聚合（Pipeline 终态 = Model-visible 口径；FNV-1a 12hex 跨版本稳定）/ os_context `OS_HASH_EPOCH` 稳定核（秒级时间行=恒真噪声，冻结后哈希只随真实环境变）/ session_events 第 14 kind `context_breakdown`（一条事件装整回合：segments+指纹三元组+rounds 逐轮序列；emit 单点在 stream_loop wrapper，恒落 turn_ended 后）/ `loop/turn_cost.rs` attribute_miss 判定表（7 slug；门槛 prompt≥1024 且 cached==0；跨回合基线 last_breakdown_payload last-wins）。fingerprint.tools 取轮 0（回合起点基线——末轮是相关性裁剪子集，比对恒真噪声）。前端五件：types/missHint（slug 中文词表两边镜像；机理说明只住前端——后端只判事实）/BudgetPill miss chip（仅 first_request→null 弱展示）/useTrajectory 折头/TrajectoryInspector 组成区（主色单色条形+偏差行+逐轮徽+披露脚注）。⚠️ 不变式：归因措辞诚实（本地推断非 provider 事实）；词表改动三处同步（event_log 词表 + derive skip 臂 / EV_KIND_TO_FILTER 与 summarizeEvent / miss_slug 与 missHint）。cargo 1465 / vitest 501
- **0.7 批 B MA-3 跨会话通讯已落地（2026-09-09，cacaa70 后端地基 + b9bea59 通道引擎 + 083f32a 前端接线 + 文档 commit；已随 0.7.0 发版）**：`send_message_to_session` 异步对等通道——pending 只入事件队列（不写 messages，与在途回合零冲突）、消费复用 run_agent_turn 全链路物化带来源前缀的 user 消息；收件三态 hold 默认（migration 52）、触发三源、expect_reply 回投链一次止、护栏常量四件；前端 useInbox/收件箱 popover/侧栏 badge/incoming 卡/轨迹 CROSS 行。详见「跨会话通讯（MA-3）」节。cargo 1443 / vitest 484
...
- `cargo test --lib` 1496 passed / 0 failed（+ 集成测试：session_runner_e2e 12、session_reconcile_e2e 6+2 ignored、session_event_log_e2e 3、memory_e2e 3、message_repo 7、provider 11）；clippy --tests -D warnings 0 警告；vitest 567（1496/567 = 频道批 C 1488/547 + 实测修复批 +5 前端[ChatMessages ⑤ 回归锁] + ② 简报重构两测试替换净 +1 Rust + ⑧ 接力去重回归锁 +1 Rust + ⑨ 同秒平局挂组回归锁 +1 前端 + ⑩ 群聊布局锁 +1 前端 + ⑪⑫ 吸入形态三件套 +2 前端 + ⑬ 吸入延递 +2 前端 + ⑭ 轮次划分回归锁 +1 Rust +1 前端 + ⑮ 同轮二段不膨胀 + election/coordinator 轮外段 +2 前端）
