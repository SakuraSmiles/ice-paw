...
- **channel_hint 段**：`context/system_prompt.rs` 频道协作纪律常量（仅 kind='channel' 由 runner 填充）——@ 礼则（提名字≠指令、只认精确成员名）、接力纪律、诚实边界、**delegate 可见性事实（子会话对频道不可见，成员成果进频道用 @）**（2026-09-11 ⑥ 补——delegate 频道同权是刻意设计[任务分发与频道接力正交]，可见性差异走 prompt 披露而非禁用）；anatomy 已收编为 `system_channel_hint` 段（0.7 批③ 组成体检自动覆盖）。
