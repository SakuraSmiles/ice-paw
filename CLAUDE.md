...
## 当前状态（2026-09-09）
- **0.7 批 A 首件已落地 db29be7（2026-09-09，发版后 commit push 未手测）**：委派子会话降级链继承——`delegation_fallback_plan`（子 agent 自链优先，无链继承父 agent 配置链并摘除子主档同 id 档；父行读失败降级无链不阻塞委派）。核查发现子 agent 自链 0.6.15 已接线（delegate 与主链共用 `production_fallback_plan`），真缺口仅「无链继承」。cargo 1433
